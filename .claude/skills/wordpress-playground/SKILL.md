...
`@wp-playground/cli` and Playground itself ship updates frequently. Before claiming a flag, a step, or a feature behaves a specific way:

| Surface | Live source |
|---|---|
| Playground docs | `https://wordpress.github.io/wordpress-playground/` |
| CLI source | `https://github.com/WordPress/wordpress-playground/tree/trunk/packages/playground/cli` |
| Blueprint schema | `https://playground.wordpress.net/blueprint-schema.json` |
| `npm view @wp-playground/cli` for current version | `npm view @wp-playground/cli version` |

A negative claim ("`--auto-mount` doesn't work with X") requires a live check.
