...
description: Author, edit, validate, or review WordPress Playground **blueprint JSON** — declarative env setup for Playground (web + CLI). Use whenever working with blueprint.json, the `$schema: playground.wordpress.net/blueprint-schema.json`, top-level fields (`landingPage`, `preferredVersions`, `features`, `extraLibraries`, `constants`, `plugins`, `siteOptions`, `login`, `steps`), step types (`installPlugin`/`installTheme` with `pluginData`/`themeData`, `activatePlugin`, `writeFile`, `writeFiles`, `runPHP`, `wp-cli`, `runSql`, `setSiteOptions`, `defineWpConfigConsts`, `enableMultisite`, `importWxr`, `request`, `updateUserMeta`, `runWpInstallationWizard`, `resetData`), resource types (`wordpress.org/plugins`, `wordpress.org/themes`, `url`, `git:directory`, `literal:directory`, `bundled`, `zip`), and bundle layout (zipped plugins/themes alongside blueprint.json). Trigger on "blueprint", "Playground config", "set up demo WordPress", "test plugin in Playground", "share WP environment", `installPlugin` / `runPHP` / `wp-cli` step debates, or any JSON destined for `playground.wordpress.net`. Pair with `wordpress-playground` to actually run the blueprint via `@wp-playground/cli`. Don't use for general WP setup (use `wordpress-playground`) or production env setup (Playground is ephemeral SQLite, never production data).
...
Blueprint schema evolves. Before claiming a top-level field, step, or resource type doesn't exist (or asserting one does), check the live schema — don't rely on training data:

| Surface | Live source |
|---|---|
| Blueprint JSON schema (canonical) | `https://playground.wordpress.net/blueprint-schema.json` |
| Blueprint docs | `https://wordpress.github.io/wordpress-playground/blueprints` |
| Step API source | `https://github.com/WordPress/wordpress-playground/tree/trunk/packages/playground/blueprints/src/lib/steps` |
| Live Blueprint Editor (paste JSON, see schema errors) | `https://playground.wordpress.net/builder/builder.html` |

Common-mistake table at the bottom of this skill is current as of late 2025; always defer to the live schema.
