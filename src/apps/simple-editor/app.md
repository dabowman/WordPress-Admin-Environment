...
- **Custom post type support.** The REST base is now derived from the post-type entity (`getPostType(postType)?.rest_base` via `useSelect`) so CPTs with a custom `rest_base` route correctly for draft creation, the autosaves endpoint, and `backHref`. The classic `post`/`page` ternary is kept as a fallback while the entity is still loading. A CPT must have `show_in_rest: true` for this editor to function — without a REST API it cannot save.
