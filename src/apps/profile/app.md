...
`useEntitySave` is called with entity coords (`{ kind: 'root', name: 'user', recordId: userId }`). This means a REST 4xx/5xx on the profile fields (e.g. invalid email) is caught via `getLastEntitySaveError` and surfaces as an error banner. Password-save failures are detected via `saveEntityRecord`'s `undefined` return value (it does **not** throw on REST error) — on failure `getLastEntitySaveError('root','user',userId)` provides the server message, which is shown as an error banner and the password fields are left filled for retry.
...
3. **Step 2 — Password (when entered):** call `await saveEntityRecord('root','user',{ id: userId, password: newPassword })` directly — the record object carries `id` so core-data issues a PATCH update, not a POST create. `saveEntityRecord` resolves `undefined` on a REST failure (does **not** throw); the return value is checked and, on falsy, `getLastEntitySaveError('root','user',userId)` provides the server message. On failure: show error banner + return (password fields remain filled for retry).
4. On full success: if profile edits were saved (Step 1 ran), `useEntitySave` already showed the success snackbar ("Profile updated."). If only a password was saved (Step 1 skipped), `createSuccessNotice('Password updated.', { type: 'snackbar' })` is called manually. Password fields are cleared.
...
- **No two-factor.** Out of scope (no core REST surface).
