...
The title-source-of-truth distinction matters: `useEntityRecord('root','site').record.title` is the canonical source; `window.wpAdminWorkspaces.siteName` is a fallback for the brief window before core-data hydrates. A prior bug had the site-hub render the window-global value indefinitely, missing user-edits from the settings page until refresh.
