...
- `GET /site-health/tests` — `require_once`s the wp-admin Site Health classes (REST context doesn't autoload them) and calls `WP_Site_Health::get_instance()->get_tests()` (which applies the `site_status_tests` filter). It runs each `['direct']` test through `WP_Site_Health::perform_test()` synchronously (which applies the `site_status_test_result` filter, so plugin amendments classic honors aren't dropped — and resolves a string `test` to its `get_test_{slug}` method first, matching core's order), and returns the results as `direct[]`, plus an `async[]` **registry** (`{ id, label, has_rest }`) describing the async tests. This is the surface that de-hardcodes the client's test list and unblocks the ~22 direct tests + plugin extensibility.
