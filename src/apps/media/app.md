...
- `buildQueryArgs(view, QUERY_MAPPING, staticArgs)` maps the DataViews `view` → REST query args: `search → search`, `sort → orderby/order`, pagination → `per_page`/`page`, and the `type` filter → `media_type`. The **Mine** and **Unattached** toolbar toggles are pseudo-filters (no DataViews filter UI) merged as static args — `author={currentUserId}` and `parent=0` respectively.
