...
- `useEntityDataView` owns `view` + `selection`. **URL slots (#136):** the app passes `urlSlots: { page: 'paged', filters: [ { field: 'type', param: 'media_type', operator: 'is' } ] }`, so `view.page` ⇄ `?paged=N` (omitted on page 1) and the single-value `type` filter ⇄ `?media_type=<value>` round-trip through the URL — refresh / deep-link / browser-back survive, mirroring NavigationApp's `?screen=` slot. The seed reads the slots so a deep-link renders the right page/filter on first paint (no flash, no wasted page-1 fetch); `setView` writes them back; a route-reconcile effect tracks back/forward. The author / date filters and the Mine / Unattached toolbar toggles stay in local state (they aren't single-value `is` slots). Pure mapping helpers in `_shared/dataviews/viewUrlSlots.mjs`, pinned by `tests/runtime/dataviews-shared.test.mjs`.
...
- **#136 URL slots** — `view.page` (`?paged`) and the `type` filter (`?media_type`) are now URL-backed via the `urlSlots` opt-in on `useEntityDataView` (the documented swap point). Remaining transient axes (search, author/date filters, `editingId` open-detail) stay local; layout / sort / perPage persist via user-prefs instead.
...
- **Partial URL state (#136 done for page + type).** `view.page` and the single-value `type` filter now round-trip through `?paged` / `?media_type` (deep-link + back/forward restore them). The author / date filters, search, and the open-detail item remain local `useState` — multi-value / range filters don't round-trip cleanly through one query param and the open-detail modal is transient; those are intentionally out of scope for this slot mechanism.
