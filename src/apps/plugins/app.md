...
The DataViews spec — fields, default view, default layouts, actions — comes from the dataView primitive at `(root, plugin, <variant|_default>)`. The app's `dataView` block in `app.json` is the baseline; workspace.json `settings.dataViews.root.plugin.<variant|_default>` and the `wp_admin_workspaces_data_view_config_root_plugin[_<variant>]` filter override per-field. The hook call is `useDataView(screenId)`.
...
Error handling is **terminal**: when a mutation fails, the app replaces the table with an error notice and stays there until the next action attempt clears it. This is more conservative than other DataViews apps in the workspace (which surface dismissible banners) — plugin-state corruption is high-consequence enough that a noisy error feels right.
...
- During refetch, the table can either show the previous state (optimistic) or a spinner (pessimistic). Workspace uses the latter via `isLoading`.
