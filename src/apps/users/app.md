...
- **Delete reassignment target is choosable** (`ReassignSelect`) on both single-row and bulk delete, defaulting to the acting user. wp-admin also offers a "Delete all content" radio alongside the reassign option; that is **not** surfaced — the REST `reassign` parameter is mandatory and has no "delete content" equivalent (the endpoint requires a valid target user id), so only reassignment is offered. The candidate list is capped at the first 100 users (no type-ahead search yet); a searchable picker is a future refinement.
