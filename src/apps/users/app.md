...
5. When there is at least one valid target, the modal renders **`ReassignSelect`** — a reassign-target dropdown that controls who inherits the deleted users' content. It defaults to the acting user (the historic behavior) and excludes the users being deleted (reassigning to a deleted account would re-orphan the content). The same modal backs both the bulk action and the per-row delete (DataViews uses one `RenderModal` for single + bulk), so the chooser surfaces on per-row delete too.

The reassign chooser is threaded through the shared `createBulkConfirmModal` factory via two optional hooks added for this: `initialControlState` (seeds the modal-local reassign-target state to `currentUserId`) and `renderControls({ targets, value, setValue })` (renders `ReassignSelect` between the message and the buttons). The selected id reaches the per-item `mutate(item, reassignTo)` as its second argument. Existing callers (posts / media / comments / taxonomy / plugins) pass neither and are unaffected — `mutate` simply ignores the extra arg.

`ReassignSelect` fetches candidates with a dedicated `useEntityRecords('root', 'user', { per_page: 100, _fields: 'id,name', context: 'view' })` (the list query can't be reused — it's paginated), excludes the delete set, and guarantees the acting user is selectable (labeled "… (you)" via `window.wpAdminWorkspaces.user.displayName`) even if they fall outside the first page of candidates.

On confirm: `Promise.allSettled(targets.map(deleteEntityRecord(..., { force: true, reassign: <selected target ?? currentUserId> })))`, then `invalidateResolution('getEntityRecords', ['root', 'user', queryArgs])`. The result is bucketed: zero failures → success snackbar; partial failure → dismissible error notice with failed/total counts; all-failed → dismissible error notice with the first rejection's message.
...
- **`change-role` bulk action shipped** ("Change role to…", `promote_users`-gated, self-demote-guarded). The role chooser is sourced from the resolved `roles` field `elements`; if a workspace ships no elements, the action falls back to the standard WordPress roles.
- **Delete reassignment target is choosable** (`ReassignSelect`) on both single-row and bulk delete, defaulting to the acting user. wp-admin also offers a "Delete all content" radio alongside the reassign option (`reassign: false` server-side, which makes `delete_item()` call `wp_delete_user( $id, null )` and delete the authored content); that is intentionally **not** surfaced — keeping the destructive path to a single safe outcome (always reassign). The candidate list is capped at the first 100 users (no type-ahead search yet); a searchable picker is a future refinement.
