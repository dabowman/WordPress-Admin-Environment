...
`data` is a `useMemo` projection of `records` into the row shape DataViews wants (`{ id, title, status, date, date_gmt, modified, sticky, passwordProtected, author, link, rawRecord }`). `date_gmt` (always UTC, ends in `Z`) is carried alongside `date` for timezone-stable missed-schedule detection — `postDateLabel` prefers `date_gmt` for the `future` comparison because `date` (site-local, no timezone suffix) would be parsed as browser-local time and the cut-over shifts with the visitor's timezone. The original record is kept on `rawRecord` so future row actions can read fields the projection doesn't surface.
