...
`data` is a `useMemo` projection of `records` into the row shape DataViews wants (`{ id, title, status, date, date_gmt, modified, sticky, passwordProtected, author, link, rawRecord }`). `date_gmt` carries no timezone designator — WordPress REST emits `Y-m-dTH:i:s` (no trailing `Z`); `postDateLabel` appends `Z` before parsing so it is treated as a true UTC instant. It is carried alongside `date` for timezone-stable missed-schedule detection — `postDateLabel` prefers `date_gmt` for the `future` comparison because `date` (site-local, no timezone suffix) would be parsed as browser-local time and the cut-over shifts with the visitor's timezone. The original record is kept on `rawRecord` so future row actions can read fields the projection doesn't surface.

**State badges + status-aware date (issue #137).** The title cell renders `_post_states`-style badges next to the title link — **Sticky** (from the `sticky` REST field) and **Password protected** (from a non-empty `edit`-context `password`). The date column renderer is status-aware, mirroring wp-admin's `column_date()`: `publish` → "Published" + publish date; `future` → "Scheduled" (or **"Missed schedule"**, in red, when the scheduled time is already past) + scheduled date; everything else (draft / pending / private) → "Last Modified" + the `modified` date. The label/field-selection logic is the pure `_shared/postDateLabel.mjs` (pinned by `tests/runtime/read-side-wins.test.mjs`); the React layer formats the chosen date via `@wordpress/date` `dateI18n`.

**Editor links (Tier 1 handoff, `docs/block-editor-native-port.md`).** The row-title anchor and the Edit action resolve their target via `editTargetHref(postType, id, routes)` ([`_shared/navigation/editorHref.mjs`](../_shared/navigation/editorHref.mjs)), with `routes` read from `useKernel().config.routes`. When the active workspace declares a primary `/{segment}/{param}/edit` route (e.g. `single-pane-demo`'s native `core:simple-editor` screens), the link is the workspace hash and stays in the SPA. When it declares none (the `wp-admin-default` Tier 1 shape), the link is the classic relative `post.php?post={id}&action=edit` — a real top-level navigation to the actual block editor that the admin-link interceptor passes through. The title is a real anchor (`Button render={<a/>}`); the Edit action callback has no anchor to click, so it goes through `followHref`, which synthesizes the anchor click rather than assigning `window.location` (keeping the interceptor in charge — see the "workspace links never bypass the admin-link interceptor" rule).
...
- Site-editor post types (`wp_template`, `wp_block`, `wp_navigation`) navigate through `editHref()` (`src/apps/_shared/navigation/editorHref.mjs`), which routes each non-core post type under a segment matching its post type id (`/wp_template/{id}/edit`, etc.) and `encodeURIComponent`-encodes the id — so a slug-shaped id like `theme//slug` collapses into one safe route segment (`theme%2F%2Fslug`) instead of splitting the hash and breaking `matchRoute` (issue #21). A workspace mounting PostsApp over such a post type still has to declare the matching `/<postType>/{id}/edit` route (without one, the Tier 1 fallback emits a classic `post.php` href, which can't edit slug-shaped ids) and the editor app must `decodeURIComponent( config.id )` before use.
...
- **Pinned status screens are locked (#209).** Trash / Drafts / Pending screens pin a status via `config.status` + the variant's `defaultView.filters`. The status field is made non-filterable (`lockStatusField` strips its `filterBy` — it stays a display column), the status view-tab strip is hidden, and the query is hard-pinned (`applyStatusPin`) so a stale or programmatic filter can't fetch a contradicting status. This keeps status-gated row actions (edit / view / trash on Trash) from reappearing on a screen labelled for another status, matching classic wp-admin which locks each of these views to its status. The lock helpers live in [`_shared/dataviews/pinnedStatus.mjs`](../_shared/dataviews/pinnedStatus.mjs) (pure, reusable by other entity-CRUD list apps — e.g. comments Spam / Trash — and pinned by `tests/runtime/dataviews-shared.test.mjs`).
...
- Title-cell **state badges** (Sticky / Password protected) and a **status-aware date column** (Published / Scheduled / Missed schedule / Last Modified) are now wired (#137). Remaining `_post_states` gaps: no Front Page / Posts Page badge (needs a `/wp/v2/settings` cross-read) and no Private / Draft badge (status is already a column).
