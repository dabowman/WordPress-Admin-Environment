...
`data` is a `useMemo` projection of `records` into the row shape DataViews wants (`{ id, title, status, date, modified, sticky, passwordProtected, author, link, rawRecord }`). The original record is kept on `rawRecord` so future row actions can read fields the projection doesn't surface.
