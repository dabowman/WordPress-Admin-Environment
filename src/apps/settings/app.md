...
Native panels (general, writing, reading, discussion) are React components that hit core-data directly. Iframed panels (permalinks, media, privacy) wrap `wp-admin/options-{permalink,media,privacy}.php` with chrome stripped. The decision matrix is documented in `docs/workspace-api-validation.md#core-settings` — when REST coverage exists for a setting, it's native; when it doesn't, the iframe pattern preserves the right user-visible outcome at the cost of some workspace-chrome polish.
