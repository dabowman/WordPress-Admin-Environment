...
- **Site Icon picker (#170).** Shipped via the shared media-library-picker control (`src/apps/_shared/forms/controls/MediaPicker.js`, wrapping `@wordpress/media-utils` `MediaUpload`). Reads/writes the `site_icon` core setting (integer attachment id) through `useEntityRecord('root','site')`. PHP `wp_enqueue_media()` provides the modal. The favicon / Apple-touch-icon preview grid that classic wp-admin renders is **not** reproduced — the picker shows the chosen attachment's thumbnail only.
