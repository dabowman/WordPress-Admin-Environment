...
- **Comments / Pings filter.** A `type` field (filter-only — declared in the dataView `fields[]` but not in `defaultView.fields`) carries `filterBy: { operators: ['is'] }`; the app supplies translated Comments / Pings options via `elementFallbacks.type`. `queryArgs` maps the selected value to the REST `type` collection param. The base query now pins `type: 'comment'` explicitly so pingbacks/trackbacks (and the WP 6.9+ `note` type) never leak in unless the filter selects **Pings** (`pings` → pingbacks + trackbacks).
