...
## "In response to" column + Comments/Pings filter (issue #137)

- **"In response to" deep-link.** A `response` column (between Comment and Status) names + links the parent post. The query carries `_embed: 'up'`, so each comment record arrives with its post embedded at `_embedded.up[0]`; the `data` projection lifts `postTitle` / `postLink` / `postType` from it (no per-row fetch). `ResponseCell` links the title to the parent post's editor via the shared `editTargetHref` resolver — the workspace editor route (`#/posts/{id}/edit` / `#/pages/{id}/edit`) when the active workspace ships one, else the classic `post.php?post={id}&action=edit` handoff (Tier 1, the same path PostsApp uses), routed through the kernel admin-link interceptor — and offers a "View Post" permalink. `wp-admin-default` declares no editor screens, so the title resolves to the classic editor there.
- **All comment types / Comments / Pings filter.** A `type` field (filter-only — declared in the dataView `fields[]` but not in `defaultView.fields`) carries `filterBy: { operators: ['is'] }`; the app supplies translated All comment types / Comments / Pings options via `elementFallbacks.type`. `queryArgs` maps the selected value to the REST `type` collection param. The base query pins `type: 'comment'` explicitly so pingbacks/trackbacks (and the WP 6.9+ `note` type) never leak into the default view; selecting **Pings** (`pings` → pingbacks + trackbacks) overrides it, and selecting **All comment types** (empty value) `delete`s the `type` arg so `WP_Comment_Query` returns every type.
...
- The "In response to" column links the parent post (issue #137) but the approved/pending **per-post comment-count bubbles** wp-admin shows are not reproduced — there's no aggregate REST source for per-post pending counts (upstream gap).
- The Comments / Pings filter (issue #137) covers comment vs. pingback/trackback, but there is still no **author-IP** filter (REST exposes `author_ip` as a read field, not a collection param — upstream gap).
