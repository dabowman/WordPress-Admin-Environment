...
- **"In response to" deep-link.** A `response` column (between Comment and Status) names + links the parent post. The query carries `_embed: 'up'`, so each comment record arrives with its post embedded at `_embedded.up[0]`; the `data` projection lifts `postTitle` / `postLink` / `postType` from it (no per-row fetch). `ResponseCell` links the title to the workspace post editor (`#/posts/{id}/edit` or `#/pages/{id}/edit` via `postEditHref`, handled by the kernel router) and offers a "View Post" permalink. Comments on post types without a workspace editor route fall back to plain text + the permalink.
