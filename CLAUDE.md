...
- **v3 dataview-registry restoration in progress** on `feat/v3-dataview-registry` (branched off `feat/wp-admin-shell-v3`). Phase 3b's view-config primitive collapsed the CIAB 3-axis registry; restoration plan + execution at `docs/plans/2026-05-20-dataview-registry-restoration.md`. Stages 0–5 shipped (schemas + fixtures + PHP resolver + REST + JS runtime + manifests reshaped); Stage 6 (cleanup sweep + deprecation shims) in progress. Vocabulary: `view-config` → `dataView`, `viewConfigs` → `settings.dataViews`, `fieldCollections` → `settings.dataFields`, `useViewConfig`/`useScreenView` → `useDataView`. Filter: `wp_admin_shell_view_config_*` → `wp_admin_shell_data_view_config_*[_{variant}]`. v2 callers keep working through one release cycle via deprecation shims.
...
**Theming model (4 tiers, DS-neutral kernel).** `src/runtime/styles/ThemeProviderHost.js` is the kernel's single seam to whatever ThemeProvider the active engine ships. Engines export an optional `ThemeProvider` field on their `EngineSource`. When the field is absent the host renders children pass-through (no inner provider) — the kernel does NOT import a DS-specific default. `WpdsThemeProvider` lives in `src/runtime/engines/core-default/WpdsThemeProvider.js` as that engine's contribution; `core-single-pane` and `core-desktop` import the same module from their sibling location (all three are WPDS-flavored bundled engines). A non-WPDS engine plugin (Material, Tailwind, brand-locked) ships its own `ThemeProvider` from its own engine bundle and is unaffected by what the bundled engines do. `WpdsThemeProvider` unlocks the real `@wordpress/theme.ThemeProvider` by piggybacking on `@wordpress/edit-site`'s allowlist entry (the package's `__dangerousOptInToUnstableAPIsOnlyForCoreModules` does string-match-only verification), then `unlock(wpTheme.privateApis).ThemeProvider`. Sites without Gutenberg + WPDS engines render empty (no fallback path — removed at v2.0.0-beta.2; the `Requires Plugins: gutenberg` header advertises the contract for the bundled WPDS engines specifically). The host wraps the inner provider in a render-error boundary; if a third-party engine's provider throws during render, the host renders children pass-through (no inner provider) and logs a console warning — engines that crash get no recovery beyond the structural pass-through, by design. **Engine-pluggable style compiler:** the engine source MAY ship a `compileStyles(styles, tokens) → {top, scoped, subtrees}` hook (`src/runtime/engines/core-default/compileStyles.mjs` for the WPDS-flavored compiler). The host calls it and emits the three buckets as a sibling `<style>` block scoped to a wrapper `<div data-shell-theme-id={id}>`. Engines omitting the hook get zero scoped overrides — provider owns all token plumbing directly. Author customization paths, in order of preference: (1) **seeds** under `styles.theme.{color.{primary,bg}, cursor.control, density}` — engine ThemeProvider's responsibility to interpret; (2) **nested seeds** under `styles.regions[id].theme` and `styles.applications[id].theme` — `<Region>` and `<MountedApp>` wrap content in nested `<ScopedThemeProvider>` reading the engine's provider from kernel context; (3) **direct slot overrides** under `styles.{color,border,dimension,elevation,font}` (top-level + per-region/app) — escape hatch the engine's `compileStyles` translates into provider-scoped CSS variables; (4) **DTCG `tokens.json` primitives** — independent of ThemeProvider, useful as named primitives consumable from any of the above via `{tokens.x.y}` aliases. The DTCG tokens table is skipped from JS serialization entirely (PHP `wp_admin_shell_styles_reference_tokens()` short-circuit in `wp-admin-shell.php`) when the resolved styles tree references zero foreign aliases — installs shipping seeds + slot overrides only carry no DTCG payload on the wire. Chrome extension layer (`--wp-admin-shell--chrome--*`) stays parallel for shell-only concepts WPDS doesn't cover (sidebar width, site-hub icon size, etc.) — bundled-engine-only concern, lives in core-default's compiler. Chrome → WPDS bridge covers four surfaces (`canvas`, `sidebar`, `toolbar`, `site-hub`); `canvas` was added 2026-05-20 so `@wordpress/ui` content rendered directly under `.wp-admin-shell-layout` retheme automatically. The `core:default` engine's `default-styles` block ships the "dark chrome + light cards" identity baseline (`chrome.{canvas,sidebar,toolbar}.background: #1e1e1e` + foreground `#e0e0e0`; `theme.color.bg: #ffffff` keeps the WPDS surface ramp light so `core:main` / `core:detail` cards paint bright). Consuming shells override per slot; the engine's `core:main` / `core:detail` templates expose `--wp-admin-shell--chrome--content--card-{background,radius,shadow,margin}` slots for the full set of card-chrome customization. Density extraction lives in `ThemeProviderHost`'s `pickDensity()` — pulls `styles.theme.density` first, falls back to legacy `styles.density`, and passes whatever string the author authored straight through to the engine's `ThemeProvider`. DS-vocabulary validation (the `default|compact|comfortable` enum) lives in `WpdsThemeProvider`, NOT the kernel — a Material/Tailwind engine receives the raw string and interprets per its own rules. **SlotFillProvider** lives in each bundled engine's Layout component, NOT the kernel — `@wordpress/components` is a DS-flavored package; a non-WPDS engine that doesn't ship `Slot`-consuming apps can omit the wrap entirely.
...
**Test surface (~1235 assertions, post-restoration).** PHP via `wp eval-file`: `run-cascade-tests.php` (39), `run-cap-tests.php` (54), `run-shape-tests.php` (126), `run-manifest-tests.php` (67), `run-tokens-tests.php` (13), `run-engine-defaults-tests.php` (22), `run-cap-gating-smoke.php` (5 — per-role nav-prune smoke for `wp-admin-default`, v2 region-tree walker), `run-chromeless-bridge-tests.php` (13 — PHP gate + body-class + script-emission contract for `core:desktop` chromeless bridge), `run-data-view-tests.php` (95 — data-field-collections registry + 3-axis data-view-config resolver `(kind, name, variant|_default)` + `extends` chain + cycle/depth caps + ref-wins-inline merge + per-base + per-variant filter machinery + variants listing + cascade contribution + `inject_app_baselines` manifest→core-origin path preserving variants + duplicate-id rejection + legacy `wp_admin_shell_view_config_*` filter shim with one-shot `_deprecated_hook` notice + REST `/screen-view` deprecation header), `run-mode-resolution-tests.php` (20), `run-v3-compiler-tests.php` (52 — v2→v3 screen synthesis + variant-from-route handling), `run-preload-tests.php` (22), `run-menu-route-shims-tests.php` (70), `run-dashboard-widgets-tests.php` (25). Node: `tests/schema/validate-shells.test.mjs` (98 — sweeps shells/manifests/engine-manifests/tokens against admin-v1 + admin-v2 + admin-app-v2 + admin-engine-v2 + admin-v3 + admin-app-v3 + admin-engine-v3 + tokens-v1, including v3 `settings.dataViews` 3-axis + `dataFields` + `extends` + `dataViewRef` fixtures), `tests/parity/wpds-snapshot.test.mjs` (4), `tests/runtime/*` (433 assertions including `data-view-merge-fields` + `data-view-hydrate-inline` for the 3-axis hydrate + extends-chain + fieldsRef merge), `tests/engines/core-desktop/*` (TS via Node `--experimental-strip-types`; 77 assertions). Browser-side perf + a11y manual passes per `docs/v1-readiness.md` + `docs/v1-perf-baseline.md`.
...
~1235 assertions — all run before merge.

```bash
# Node
npm run test:schema      #  98 — Ajv: admin-v1 + admin-v2 + admin-app-v2 + admin-engine-v2 + admin-v3 + admin-app-v3 + admin-engine-v3 + tokens-v1 sweeps (+ C2 viewConfigs / fieldCollections + C1 preload + C4 dashboardWidget / dashboardWidgets + v3 settings.dataViews / dataFields / dataViewRef / extends fixtures)
npm run test:parity      #   4 — WPDS slot-list drift detector
npm run test:runtime     # 433 — runtime modules chained: resolveRegion + validateRegion + platformServices + matchRoute + dirtyState + tokensResolver + compileStylesTokens + bindings + spec-worked-example + registry ThemeProvider + registry lazy-app + engine defaults + icon-registry + dynamicChildren store + chromeless-bridge envelope contract + shouldRenderRegion + resolver-gate pipeline + data-view mergeFields + data-view hydrateInline (3-axis triple + extends chain + fieldsRef merge) + composeWidgets + mode-resolution
...
npx wp-env run cli wp eval-file wp-content/plugins/WordPress-Admin-Environment/tests/php/run-shape-tests.php             # 126
...
npx wp-env run cli wp eval-file wp-content/plugins/WordPress-Admin-Environment/tests/php/run-data-view-tests.php         #  95
...
npx wp-env run cli wp eval-file wp-content/plugins/WordPress-Admin-Environment/tests/php/run-v3-compiler-tests.php       #  52
...
npx wp-env run cli wp eval-file wp-content/plugins/WordPress-Admin-Environment/tests/php/run-dashboard-widgets-tests.php #  25
```
...
│   │   ├── styles/                 # ThemeProviderHost (engine-pluggable seam, owns DS-neutral density extraction). Imports no DS-specific module. WpdsThemeProvider moved to `src/runtime/engines/core-default/WpdsThemeProvider.js` as that engine's contribution; `core-single-pane` and `core-desktop` import it from there. compileStyles + wpds-defaults snapshot also live in core-default. density.js retired (kernel no longer writes `data-wpds-density` — the real `@wordpress/theme.ThemeProvider` handles attribute emission internally via its `density` prop).
...
│   │   │   ├── useDataView.js      # React hook overloaded: useDataView(screenId) OR useDataView({kind, name, variant}, {fallback}) → {config, isLoading}. Inline-snapshot fast path + /wp-admin-shell/v1/data-view REST fallback. Re-exports deprecated `useScreenView` alias for one release cycle.
...
| `core:dashboard-host` | DashboardHostApp | ✅ | — | C4 widget-grid controller. Reads `window.wpAdminShell.manifests.apps` for every app declaring a `dashboardWidget` block, merges admin.json `dashboardWidgets` overrides via `composeWidgets()`, renders surviving widgets as tiles in a `display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr))` container. Per-tile `gridColumn/Row span` from each widget's `defaultSize`; explicit `{row, col}` positions become `grid-{row,column}-start`. Tiles mount the widget via `<MountedApp>` so the existing 4-layer cap gating + theming applies uniformly. Bundled mount: `/dashboard-grid` route in `developer-admin`. |
| `core:dashboard-widget-recent-posts` | DashboardWidgetRecentPostsApp | ✅ | `edit_posts` | C4 example widget. Lists five most recent post drafts (`postType/post`, `status: draft`, orderby modified desc, context: edit). Click → `#/posts/{id}/edit`. |
| `core:dashboard-widget-quick-draft` | DashboardWidgetQuickDraftApp | ✅ | `edit_posts` | C4 example widget. Title + textarea + Save Draft button. On submit creates a draft via `saveEntityRecord`, invalidates the recent-drafts query, navigates to `#/posts/{id}/edit`. Empty body seeds `<!-- wp:paragraph --><p></p><!-- /wp:paragraph -->` to satisfy WP's empty-post rejection. |
...
Thirteen extension surfaces, all in place:
