...
- **C5 lazy app registration** landed on `feat/c5-lazy-app-loading` (2026-05-14). Registry's `register()` accepts both eager `{ Component }` and lazy `{ load: () => import(...) }` shapes; mount path awaits the load on first match through `registry.resolveComponent(id)` → `React.lazy()` wrapped in `<Suspense>` + `<AppErrorBoundary>`. 25 of 30 bundled apps converted to dynamic `import()` with `webpackChunkName: "app-<id>"` magic comments — each emits its own `build/app-<id>.js` chunk. Always-eager: `core:navigation`, `core:site-hub`, `core:toolbar-actions`, `core:notices-banner`, `core:notices-snackbar` (mounted in every shell, lazy adds flicker). **Cold-mount JS dropped 90 %**: `index.js` 2.06 MiB → 209 KiB; webpack auto-extracted a 1.75 MiB vendors chunk that loads on-demand when the first DataViews-backed app mounts. See `docs/plans/track-d-lazy-app-loading.md` + `docs/v1-perf-baseline.md` Bundle-size table.
- **C1 REST preload** landed on `feat/c1-rest-preload` (2026-05-14). New top-level `preload[]` block in admin-v2 — string paths or `[ path, method ]` tuples (GET / OPTIONS). `WP_Admin_Shell_Preload::inject()` walks every cascade origin's `preload[]` through the existing `wp_admin_shell_data_{origin}` filters, concatenates additively, dedupes by `path|method`, hydrates via `rest_preload_api_request`, and ships the cache as inline script on `wp-api-fetch` (`wp.apiFetch.use( wp.apiFetch.createPreloadingMiddleware( ... ) )`) before the shell bundle enqueues. Cascade is additive only — preload entries carry no user-meaningful identity, so site/role/user simply append; no override semantics. Per-entry `Throwable` catch isolates failures so one bad path doesn't poison the bundle. Conditional preloads belong in a `wp_admin_shell_data_{origin}` filter callback. CIAB-compat shape — plugins porting from CIAB drop their hardcoded array straight into admin.json. Spec §13 #9 + extension-points list grow to nine.
- **C4 dashboard widget grid** lives on `feat/c4-dashboard-grid` (2026-05-14, branched from main). New `core:dashboard-grid` region template in `core-default/engine.json` (auto-fill CSS grid, declares `core:dynamic-children` for future use); new `core:dashboard-host` app reads every manifest carrying a `dashboardWidget` block + admin.json `dashboardWidgets` overrides via `composeWidgets()` and renders the survivors as tiles inside its own grid container (works in any region, not just the dashboard-grid template). Two bundled example widgets: `core:dashboard-widget-recent-posts` (drafts list, ported from `core:dashboard`'s recent-drafts card) + `core:dashboard-widget-quick-draft` (Substack-style mini form). PHP `wp_admin_shell_register_dashboard_widget( $id, $args )` supports an override-only call (sets `position`/`defaultSize`/`minSize`/`title`/`hidden` for an already-registered app) and a standalone call (also synthesizes a minimal app manifest from `script` + `role` + `capabilities` + `dashboardWidget` so a mu-plugin can drop a widget without a separate `wp_admin_shell_register_app()` call). The synthetic manifest is forwarded to `wp_admin_shell_register_app()` at `init` priority 7 (lazy-flushed earlier on first `wp_admin_shell_data_plugin` apply) so plugin authors can register from mu-plugin top-level or early `plugins_loaded` before the manifest-registry class loads. Two distinct merge layers: (1) admin.json `dashboardWidgets[id]` wins **entirely** over the PHP-registered override for the same id (entry-replacement, same pattern as `fieldCollections`); (2) the resolved override then merges **per-property** over the app manifest's `dashboardWidget` block at render time via `composeWidgets()` — admin.json winning each field. Widgets are apps, so the existing 4-layer cap gating + theming + source-cap floor applies without a new gating layer. `developer-admin` ships a `/dashboard-grid` demo route mounting `core:dashboard-host`; the legacy `/dashboard` route + `core:dashboard` app still ship — promotion deferred to a follow-up after stability. See `docs/plans/track-c-dashboard-grid.md` + spec §13 #13.
...
1021 assertions — all run before merge.
