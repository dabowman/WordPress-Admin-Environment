...
npx wp-env run cli wp eval-file wp-content/plugins/WordPress-Admin-Environment/tests/php/run-alpha-routing-tests.php       # workspace hijack decision (root-entry / allowlist / context guard), classic-mode cookie + admin-bar node, classic→workspace legacy-redirect mapping
```
