...
894 assertions — all run before merge.

```bash
# Node
npm run test:schema      # 81 — Ajv: admin-v1 + admin-v2 + admin-app-v2 + admin-engine-v2 + tokens-v1 sweeps (+ C2 viewConfigs / fieldCollections + C1 preload fixtures)
...
npx wp-env run cli wp eval-file wp-content/plugins/WordPress-Admin-Environment/tests/php/run-preload-tests.php          #  20
```
...
Nine extension surfaces, all in place:
