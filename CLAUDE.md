...
- **Self-delete guard on bulk user delete.** Filter out the acting user (`window.wpAdminWorkspaces?.userId`) before sending REST. Reassign-to-self fails server-side and the bulk request errors silently mid-flight.
...
42 bundled apps under `src/apps/` — `core:posts` / `simple-editor` / `editor` (iframe) / `media` / `taxonomy` / `profile` / `users` / `user-new` / `comments` / `menus` / `settings` (composable host) / `settings-general` / `settings-writing` / `settings-reading` / `settings-discussion` / `settings-media` / `settings-workspace` (standalone native panels, also imported by the host) / `plugins` / `themes` / `tools` / `site-health` / `site-editor` (iframe) / `appearance-preferences` / `iframe-fallback`, the system apps (`navigation`, `site-hub`, `toolbar-actions`, `command-palette`, `preview-pane`, `notices-banner`, `notices-snackbar`, `user-menu`), the dashboard host + bundled widgets (`dashboard-widget-{activity,at-a-glance,classic,quick-draft,recent-posts}` — the old `core:dashboard` monolith is retired), and the `core:desktop-*` engine apps (`desktop-compositor`, `desktop-dock-app`, `desktop-iframe`, `desktop-window-frame`). The six entity-CRUD list apps (posts / taxonomy / users / comments / plugins / themes) are dataView consumers sharing `src/apps/_shared/dataviews/` scaffolding (see Key rules).
