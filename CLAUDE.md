...
**Test surface (475 assertions).** PHP via `wp eval-file`: `run-cascade-tests.php` (22), `run-cap-tests.php` (54), `run-shape-tests.php` (100), `run-manifest-tests.php` (60), `run-tokens-tests.php` (13). Node: `tests/schema/validate-shells.test.mjs` (54 — sweeps shells/manifests/engine-manifests/tokens against admin-v1 + admin-v2 + admin-app-v2 + admin-engine-v2 + tokens-v1), `tests/parity/wpds-snapshot.test.mjs` (4), `tests/runtime/*` (168 — resolveRegion + validateRegion + platformServices + matchRoute + dirtyState + tokensResolver + compileStylesTokens). Browser-side perf + a11y manual passes per `docs/v1-readiness.md` + `docs/v1-perf-baseline.md`.
