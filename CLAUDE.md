...
**Test surface (v2 baseline).** PHP via `wp eval-file`: `run-cascade-tests.php` (22), `run-cap-tests.php` (54), `run-shape-tests.php` (83), `run-manifest-tests.php` (60). Node: `tests/schema/validate-shells.test.mjs` (50 — admin-v1 + admin-v2 + admin-app-v2 + admin-engine-v2; bundled v2 shells, the 19 `src/{,runtime/}apps/*/app.json` manifests, and the bundled `engine.json` all sweep under their respective v2 schemas), `tests/parity/wpds-snapshot.test.mjs` (4), `tests/runtime/*` (130 — resolveRegion + validateRegion + platformServices + matchRoute + dirtyState). 403 total. Browser-side perf + a11y manual passes tracked in `docs/v1-readiness.md` and `docs/v1-perf-baseline.md`; v2 will evolve these in place.
