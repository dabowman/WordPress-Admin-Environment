...
**Current shell coverage:** `core:themes` — a native DataViews app (`src/apps/themes/`) renders the installed-themes grid + details modal + activate action. Theme install/browse is not yet ported; the iframe escape hatch (`iframe:theme-install.php`) covers that.
