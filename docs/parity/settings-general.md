...
| **Timezone — manual UTC offset** (`gmt_offset`) | `UTC±X` options; on save, mapped to `gmt_offset` + `timezone_string` cleared (`options.php:282-296`) | `UTC±X` options injected in the same select (`wp-admin-workspaces.php`) | ✅ full | The shell writes `timezone="UTC+5"` via REST (option `timezone_string`). `gmt_offset` is not REST-registerable on its own, so a **`rest_pre_update_setting` filter** (`wp-admin-workspaces.php`) now intercepts the `timezone_string` write and mirrors `options.php`: `UTC±X` → set `gmt_offset`, clear `timezone_string`; IANA zone → set `timezone_string` and let core keep `gmt_offset` in sync with the zone (`wp_timezone()` ignores it while a zone is set). No more silent revert. |
...
2. **Date-format preset count (4 vs 5).**
   ...
   - Shell: `array( 'F j, Y', 'Y-m-d', 'm/d/Y', 'd/m/Y' )` (`wp-admin-workspaces.php:1190`).
...
3. **Default-role dropdown contents.**
   ...
   - Shell: lists every role from `wp_roles()->get_names()` (`wp-admin-workspaces.php:1180-1187`), unfiltered.
...
5. **Universal/Local time is a snapshot.**
   ...
   - Shell: injects `utcNow`/`localNow` once at page load (`wp-admin-workspaces.php:1214-1215`), always shows both (`index.js:301-307`).
...
1. ✅ **RESOLVED (shell shim) — `home` (Site Address) is not REST-registered in core.** `[shell]`
   ...
   - Resolution: the shell `register_setting('general','home', ['show_in_rest'=>['name'=>'home','schema'=>['format'=>'uri']], 'type'=>'string'])` (`wp-admin-workspaces.php`, non-multisite). `editedRecord.home` now round-trips. Upstream could still register it for clean cross-client parity.
...
2. ✅ **RESOLVED (shell shim) — `users_can_register` is not REST-registered in core.** `[shell]`
   ...
   - Resolution: shell `register_setting('general','users_can_register', boolean, show_in_rest)` (`wp-admin-workspaces.php`, non-multisite). Checkbox round-trips.
...
3. ✅ **RESOLVED (shell shim) — `default_role` is not REST-registered in core.** `[shell]`
   ...
   - Resolution: shell `register_setting('general','default_role', string, show_in_rest)` (`wp-admin-workspaces.php`, non-multisite). Value round-trips. Still open: apply `default_role_dropdown_excluded_roles` when building `data.roles` (recommendation #7).
...
8. **Available-languages + timezone-city + role lists are not REST endpoints.** `[shell]` (already mitigated)
   ...
   - Shell impact: **already handled** by injecting `window.wpAdminWorkspaces.settingsGeneral` server-side (`wp-admin-workspaces.php:1109-1233`). Not a live blocker, but note the data is a page-load snapshot (no refresh).
...
6. **Add the `d.m.Y` date preset.** `[shell]` one-line fix in `wp-admin-workspaces.php:1190` to match core 7.0 (`options-general.php:498`).
7. **Apply `default_role_dropdown_excluded_roles`** when building `data.roles` (`wp-admin-workspaces.php:1180`) so admin/editor aren't offered. `[shell]` (do alongside #1).
