...
- **REST / core-data surface the workspace app uses:**
  ...
  - `window.wpAdminWorkspaces.settingsGeneral` — PHP-injected read-only metadata (`wp-admin-workspaces.php:1109-1233`): language optgroups, timezone groups, role list, weekday labels, date/time preset arrays, multisite flag, `WP_SITEURL`/`WP_HOME` constant flags, `pendingAdminEmail`, current UTC/local time strings.
- **Project screen spec:** `docs/screens/settings-general.md` — present and detailed (tier-2). It correctly flags `home`/`users_can_register`/`default_role`/`site_icon` as non-REST, but its REST-exposed table lists `d.m.Y` as a 6.8.0 preset that the workspace's injected list omits, and it predates the manual-UTC-offset-revert finding. Both are captured below.
...
| **Site Icon** (`site_icon`) | media-modal picker (≥512px), app/browser favicon previews, Choose/Change/Remove buttons, `upload_files` cap (`options-general.php:100-223`) | **absent** | ❌ missing | `site_icon` is **not REST-registered** (verified — no `register_setting` for it). Needs a media-picker primitive the workspace doesn't expose + an option shim. See blockers. |
...
| **Membership: Anyone can register** (`users_can_register`) | checkbox in `<fieldset><legend>` (`options-general.php:296-302`) | `CheckboxControl` bound to `editedRecord.users_can_register` (`index.js:220-230`) | ✅ full | core 7.0 does not REST-register it; the **workspace now shims it** (`register_setting('general',…, boolean, show_in_rest)`, non-multisite). Checkbox round-trips. |
| **New User Default Role** (`default_role`) | `wp_dropdown_roles()`, excludes admin+editor via `default_role_dropdown_excluded_roles` (7.0, `options-general.php:308-330`) | `SelectControl` bound to `editedRecord.default_role`, fallback `'subscriber'`, options from `data.roles` (`index.js:231-242`) | 🟡 partial | core 7.0 does not REST-register it; the **workspace now shims it** (`register_setting('general',…, string, show_in_rest)`, non-multisite), so the value round-trips. Remaining fidelity gap: workspace's `data.roles` = `wp_roles()->get_names()` (`wp-admin-workspaces.php`) does **not** apply the 7.0 `default_role_dropdown_excluded_roles` filter, so it still lists `administrator`/`editor` (recommendation #7). |
...
| **Timezone — manual UTC offset** (`gmt_offset`) | `UTC±X` options; on save, mapped to `gmt_offset` + `timezone_string` cleared (`options.php:282-296`) | `UTC±X` options injected in the same select (`wp-admin-workspaces.php`) | ✅ full | The workspace writes `timezone="UTC+5"` via REST (option `timezone_string`). `gmt_offset` is not REST-registerable on its own, so a **`rest_pre_update_setting` filter** (`wp-admin-workspaces.php`) now intercepts the `timezone_string` write and mirrors `options.php`: `UTC±X` → set `gmt_offset`, clear `timezone_string`; IANA zone → set `timezone_string` and let core keep `gmt_offset` in sync with the zone (`wp_timezone()` ignores it while a zone is set). No more silent revert. |
...
| **DST status + next transition** | "currently in daylight saving / standard time", "Daylight saving begins on: …" / "…does not observe DST" via `timezone_transitions_get()` (`options-general.php:441-479`) | **absent** | ❌ missing | Not shown at all. Computable client-side via `Intl`/`Temporal`, or server-injectable. Workspace-side gap. |
| **Date Format** (`date_format`) | radio presets + Custom radio + freeform input + live preview + spinner (`options-general.php:483-528`) | `RadioControl` (presets + `__custom__`) + revealed `InputControl` (`index.js:311-336`) | 🟡 partial | REST-registered; works. **Preset list drift:** core 7.0 ships 5 presets incl. `d.m.Y` (added 6.8.0, `options-general.php:498`); workspace injects only 4 (`wp-admin-workspaces.php:1190`). A site on `d.m.Y` shows as "Custom" in the workspace. **No live preview** of the custom format against current time. |
...
| **Capability gate (view)** | `manage_options` else `wp_die()` (`options-general.php:15-17`) | `manage_options` floor via PHP (`wp-admin-workspaces.php:541`) + app `role:"main"`; REST `get_item_permissions_check` re-checks `manage_options` (`controller:67-69`) | ✅ full | Server-enforced on both injection and REST. |
...
| **Multisite field hiding** | hides WP/Site URL + Membership + Default Role; **keeps** Admin Email (per-site contact, `options-general.php:264` is not MS-guarded) | hides URL, home, Admin Email, Membership, Default Role (`index.js:133,181,214`) | 🟡 partial | Workspace **over-hides**: it drops the Administration Email field on multisite, which classic still shows. (REST `email` is non-MS-only anyway, so it couldn't be saved via REST — but classic offers it via the legacy `new_admin_email` POST.) |
...
| **Help tabs / sidebar** | Overview help tab + "For more information" sidebar (`options-general.php:50-62`) | **absent** | ❌ missing | Cross-cutting workspace gap (no screen-help surface); same for every settings panel. |
...
1. **Admin email saves instantly vs confirm-by-link.**
   ...
   - Workspace: writes REST `email` → `update_option('admin_email', …)` **immediately** (`controller:202-204`). The lockout safeguard is gone.
...
2. **Date-format preset count (4 vs 5).**
   ...
   - Workspace: `array( 'F j, Y', 'Y-m-d', 'm/d/Y', 'd/m/Y' )` (`wp-admin-workspaces.php:1190`).
   - **Consequence:** the `d.m.Y` European preset is missing; a site already set to `d.m.Y` shows as "Custom" rather than a selected preset radio. Workspace-side, trivial fix.
...
3. **Default-role dropdown contents.**
   ...
   - Workspace: lists every role from `wp_roles()->get_names()` (`wp-admin-workspaces.php:1180-1187`), unfiltered.
   - **Consequence:** the workspace would offer `administrator`/`editor` as the new-user default (a footgun). Moot today only because the field can't save (see blockers), but a divergence the moment it's fixed.
...
4. **Multisite admin-email field.**
   ...
   - Workspace: hides it on multisite (`index.js:181`).
   - **Consequence:** multisite site admins lose the per-site contact-email control in the workspace.
...
5. **Universal/Local time is a snapshot.**
   ...
   - Workspace: injects `utcNow`/`localNow` once at page load (`wp-admin-workspaces.php:1214-1215`), always shows both (`index.js:301-307`).
...
6. **Tagline helper example dropped.**
   ...
   - Workspace: generic helper, no example (`index.js:123-126`). Cosmetic.

## API & platform blockers

The hard parity blockers — verified against live 7.0 source.

1. ✅ **RESOLVED (workspace shim) — `home` (Site Address) is not REST-registered in core.** `[workspace]`
   ...
   - Resolution: the workspace `register_setting('general','home', ['show_in_rest'=>['name'=>'home','schema'=>['format'=>'uri']], 'type'=>'string'])` (`wp-admin-workspaces.php`, non-multisite). `editedRecord.home` now round-trips. Upstream could still register it for clean cross-client parity.

2. ✅ **RESOLVED (workspace shim) — `users_can_register` is not REST-registered in core.** `[workspace]`
   - Evidence: grep of `register_setting(` shows no `users_can_register`; absent from `register_initial_settings()`.
   - Resolution: workspace `register_setting('general','users_can_register', boolean, show_in_rest)` (`wp-admin-workspaces.php`, non-multisite). Checkbox round-trips.

3. ✅ **RESOLVED (workspace shim) — `default_role` is not REST-registered in core.** `[workspace]`
   - Evidence: same grep — no `register_setting` for `default_role`.
   - Resolution: workspace `register_setting('general','default_role', string, show_in_rest)` (`wp-admin-workspaces.php`, non-multisite). Value round-trips. Still open: apply `default_role_dropdown_excluded_roles` when building `data.roles` (recommendation #7).

4. ✅ **RESOLVED (workspace shim) — `gmt_offset` is not REST-registered, and `timezone_string` rejects manual offsets.** `[workspace]`
...
5. **`site_icon` is not REST-registered.** `[upstream]` (option) + `[workspace]` (UI primitive)
   ...
   - Workspace impact: entire Site Icon control (favicon/app-icon previews, Choose/Remove) is missing. The workspace also lacks a media-picker primitive (per `app.md:44`).
   - Close it: `[workspace]` build a media-picker primitive + `update_option('site_icon', $id)` shim (or use the block-theme `site_logo`/Site Logo block path); the option itself is `[upstream]` for clean REST.
...
6. **Admin-email confirmation flow has no REST equivalent.** `[upstream]`
   ...
   - Workspace impact: cannot initiate a *pending* change (only an instant one) and cannot Cancel a pending one — only display it.
   - Close it: `[workspace]` custom endpoint that writes `new_admin_email` (firing the existing email side effect) + a cancel endpoint deleting `adminhash`/`new_admin_email`; or `[upstream]` make `new_admin_email` a REST-recognized "staging" write that triggers confirmation. (Either way the email send + hash check are server-only — no pure-REST path exists today.)
...
7. **Language-pack download on locale select is admin-only.** `[upstream]`
   ...
   - Workspace impact: writing REST `language` to an *Available* (uninstalled) locale stores the code but never fetches the `.mo` files; the site stays en_US.
   - Close it: `[workspace]` after saving an uninstalled locale, call a custom endpoint that runs `wp_download_language_pack()` (capability-gated on `install_languages` + `wp_can_install_language_pack()`); `[upstream]` add a download side effect to the REST `WPLANG` update.

8. **Available-languages + timezone-city + role lists are not REST endpoints.** `[workspace]` (already mitigated)
   ...
   - Workspace impact: **already handled** by injecting `window.wpAdminWorkspaces.settingsGeneral` server-side (`wp-admin-workspaces.php:1109-1233`). Not a live blocker, but note the data is a page-load snapshot (no refresh).

9. **Plugin `add_settings_field('general',…)` / `do_settings_sections('general')` extensions.** `[upstream]`/`[workspace]`
   ...
   - Close it: workspace-level "additional fields" slot on the panel (per `screens/settings-general.md:374`) — non-trivial; an accepted gap for now.
...
- **Anti-pattern resolved (issue #106):** the hand-rolled form renders four controls (`home`, `users_can_register`, `default_role`, manual-offset timezone) bound to entity fields core's REST schema doesn't register. `useEntityRecord` happily accepts `edit({home: …})` into its local edit object, but `save()` used to POST keys the controller ignored — a silent no-op with no client-side signal. The workspace now **backs them with shims** (`register_setting(show_in_rest)` for the three `general` options + a `rest_pre_update_setting` filter for the manual offset; see Recommendations #1–2), so the bindings round-trip without changing the form library or the app code.

## Recommendations / future work

**P1 — silently-broken controls (correctness; ship-blockers for an honest UI).**
1. ✅ **Done — backed the no-op controls with `register_setting` shims.** `home`, `users_can_register`, `default_role` are now `register_setting`'d on the core `general` group with `show_in_rest` (`wp-admin-workspaces.php`, the `init` settings action, `! is_multisite()` branch), so `/wp/v2/settings` exposes them and the existing `index.js:155-242` bindings round-trip. `[workspace]` — no app change needed.
2. ✅ **Done — routed the manual UTC-offset write to `gmt_offset`.** A `rest_pre_update_setting` filter (`wp-admin-workspaces.php`) intercepts the `timezone` write keyed on `option_name === 'timezone_string'` and mirrors `wp-admin/options.php`: a `UTC±X` selection sets `gmt_offset` + clears `timezone_string`; an IANA zone (or bare `UTC`) sets `timezone_string` and leaves `gmt_offset` to core's zone sync. The offset no longer reverts silently. `[workspace]` — the offset options were already injected (`wp-admin-workspaces.php`); only the write path was missing.
3. **Guard the admin-email lockout.** REST writes `admin_email` instantly (`controller:202-204`). Minimum `[workspace]`: a confirm dialog before saving an email change. Full parity `[workspace]`+`[upstream]`: write `new_admin_email` via a shim to trigger the existing confirm-by-email flow (`misc.php:1450`) and add a Cancel action (`options.php?dismiss=…`).

**P2 — visible feature gaps.**
4. **Language-pack download on save.** `[workspace]` after saving an uninstalled *Available* locale, call a shim running `wp_download_language_pack()` (cap-gated). Otherwise the locale silently no-ops (`options.php:307-316` is the only core trigger).
5. **Site Icon picker.** `[workspace]` build the media-picker primitive (per `app.md:44`) + `update_option('site_icon', $id)` shim; mirror the favicon/app-icon previews from `options-general.php:160-220`.
6. **Add the `d.m.Y` date preset.** `[workspace]` one-line fix in `wp-admin-workspaces.php:1190` to match core 7.0 (`options-general.php:498`).
7. **Apply `default_role_dropdown_excluded_roles`** when building `data.roles` (`wp-admin-workspaces.php:1180`) so admin/editor aren't offered. `[workspace]` (do alongside #1).
8. **Restore the admin-email field on multisite.** `[workspace]` — classic shows it (`options-general.php:264`); workspace over-hides (`index.js:181`). Needs a non-REST shim since REST `email` is non-MS-only.

**P3 — fidelity / polish.**
9. **DST + next-transition info.** `[workspace]` compute client-side (`Intl`/`Temporal`) or inject server-side; classic shows it at `options-general.php:441-479`.
10. **Live date/time format preview.** `[workspace]` render the selected/custom format against `new Date()` client-side; classic shows a preview span (`options-general.php:523,568`).
11. **Live-ticking UTC/local clock + hide "Local time" when no timezone set.** `[workspace]` compute client-side instead of the page-load snapshot (`index.js:301-307`).
12. **Tagline example + "Documentation on date and time formatting" link.** `[workspace]` cosmetic copy parity (`options-general.php:88-92`, `:571`).
13. **Per-field error surfacing.** `[workspace]` show server `add_settings_error`-equivalent messages (e.g. invalid timezone) instead of swallowing them in the all-or-nothing error banner.
14. **Plugin `general`-section field extension slot.** `[workspace]`/`[upstream]` — accepted gap; `do_settings_fields('general')` extensions don't render (`options-general.php:592`).
15. **Doc maintenance.** Update `docs/screens/settings-general.md` (add the manual-UTC-offset-revert blocker; note the workspace's 4-vs-5 date-preset drift) and `app.md` (the `home`/`users_can_register`/`default_role` no-op bindings are not currently listed as bugs — only the email/site-icon gaps are).
