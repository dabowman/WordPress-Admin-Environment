...
1. ✅ **Resolved (#136).** ~~Active panel lives in `useState`, not the URL.~~ The host now derives the active panel from a `?panel=<id>` URL slot (resolution order: `?panel` → path segment `segments[0]` → first panel), written by a `navigatePanel` helper mirroring NavigationApp's `navigateScreen`. Refresh / bookmark / back-button preserve the panel even when routing through the host's internal nav. (Historical: wp-admin gives each Settings page a distinct URL; the host previously read `segments[0]` only as the initial `useState` seed and never wrote back.)
...
3. ✅ **Resolved (#136), screen-side.** The `settings-privacy` screen's OR-set in `wp-admin-default.json` now lists `manage_privacy_options` ahead of `manage_options`, so a Privacy-Officer role granted only the meta-cap can reach the Privacy screen (matching classic `options-privacy.php:12`). The host panel constant still hard-codes `manage_options` (`settings/index.js`), which only matters for workspaces that route Privacy *through* the host's internal nav rather than as its own screen; the default workspace uses a standalone screen, so this is closed for it. The iframe still enforces the real classic cap regardless.
...
| 8 | ✅ **Resolved (#136).** **`manage_privacy_options` capability** | The Privacy screen's OR-set in `wp-admin-default.json` now lists `manage_privacy_options` ahead of `manage_options`. | **[done]** — the kernel's OR-semantic cap eval already supported it; it was just unlisted. |
...
**P2 — Move host active-panel state into the URL.** ✅ **Done (#136).** The host derives the active panel from a `?panel=<id>` slot via `navigatePanel` (mirrors NavigationApp's `navigateScreen`); refresh/deep-link/back-button preserve it. Closed Functional divergence #1.

**P2 — Add `manage_privacy_options` to the Privacy screen's permission OR-set.** ✅ **Done (#136).** The `settings-privacy` screen now lists `manage_privacy_options` ahead of `manage_options`, matching `options-privacy.php:12`. Closed divergence #3 / blocker #8.
