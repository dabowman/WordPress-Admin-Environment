...
2. **No theme install / .org directory browse via REST.** `[upstream]` + `[shell]`
   ...
   - **To close:** `[shell]` build a server-side proxy (`/wp-admin-workspaces/v1/themes-directory` wrapping `themes_api()`) + an install endpoint wrapping `Theme_Upgrader`, plus filesystem-credentials handling. `[upstream]` no core REST surface exists for either.
...
- `[shell]` ✅ `POST /wp-admin-workspaces/v1/activate-theme` is registered in `includes/class-wp-admin-workspaces-themes-rest.php`: permission `current_user_can('switch_themes')`, REST-nonce protected, validates the theme before calling `switch_theme()`. `src/apps/themes/index.js:132` targets this path.
...
- `[shell]` ✅ The `app.json` `data.reads[]` entry claiming a `window.wpAdminWorkspaces.activeTheme` read that doesn't exist was removed (the app derives active-ness from each record's `status`). `docs/screens/themes.md:5` "Current shell coverage: None" was stale and now reflects `core:themes`.
...
- `[shell]` Custom `DELETE /wp-admin-workspaces/v1/theme/{stylesheet}` (cap `delete_themes`, filesystem-credentials aware) + a destructive `delete` action via the shared `createBulkConfirmModal` factory. Suppress when the theme has an active child (mirror `theme.php` `$parents` logic).
