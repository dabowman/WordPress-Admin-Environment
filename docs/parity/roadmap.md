...
5. **Stop simple-editor auto-saving PUBLISHED posts to the live record.** ✅ **Done (#101).** *core:simple-editor.* The 2s debounce is now status-gated (`autosave.mjs` `autosaveTarget()`): draft/auto-draft/pending flush to the parent via `save()`; published/private/scheduled route to `POST .../autosaves`, leaving the live record untouched until an explicit Update. Recovery banner (`modified_gmt` compare on load) remains open — see block-editor.md blocker #3. ([block-editor.md](block-editor.md))
...
10. **Stop rendering silently-broken settings controls (or back them with shims).** *core:settings-general / settings-reading.* Controls for `home`, `users_can_register`, `default_role`, manual UTC offset (general) and `posts_per_rss`, `rss_use_excerpt` (reading) render, accept input, show "Settings saved.", and discard the value because those options aren't `show_in_rest`. Remove them, label them legacy, or back them with `register_setting(show_in_rest)` shims (see B-P1). ([settings-general.md](settings-general.md), [settings-reading.md](settings-reading.md))
