...
| B2 | **Personal Options prefs** (`rich_editing`, `syntax_highlighting`, `comment_shortcuts`, `show_admin_bar_front`) | All four are user meta processed by `edit_user()` from `$_POST`, but **none** is registered with `show_in_rest`. The only user `register_meta(... show_in_rest ...)` in core is `persisted_preferences` (editor UI state), not these. | [upstream] (expose via `register_meta`) **or** [shell] (register them on the shell's own `/wp-admin-workspaces/v1/user-prefs` endpoint, which currently stores only `wp_admin_workspaces_user_prefs`) | `includes/user.php:135-141`; `_get_additional_user_keys()` lists them `wp-includes/user.php:3006`; only meta with `show_in_rest` is `persisted_preferences` `wp-includes/user.php:5206-5229`. |
...
5. **Expose Personal Options prefs to REST** (`admin_color`, `rich_editing`, `syntax_highlighting`, `comment_shortcuts`, `show_admin_bar_front`). *Why:* blockers B1–B2; these are core wp-admin behaviors. *Where:* either [upstream] `register_meta(... show_in_rest ...)` in core, **or** [shell] register them on the shell's `WP_Admin_Workspaces_Prefs_REST` (`includes/class-wp-admin-workspaces-prefs-rest.php`) writing through to `update_user_meta` so a future Personal Options panel can read/write them. The shell-side bridge is the pragmatic path and keeps it self-contained. **Mixed — prefer [shell] bridge.**
