...
| Helper text "Individual posts may override these settings…" | `<p class="description">` (`options-discussion.php:62`) | Absent | 🔴 | Workspace-side, trivial. Workspace shows a different (REST-gap) notice instead. |
...
| Link to moderation queue ("moderation queue" anchor → `edit-comments.php?comment_status=moderated`) | Inline link in helper (`options-discussion.php:195`) | Absent | 🔴 | Workspace-side: should link to the `comments` app with `?status=hold`. Comments REST collection supports `status` (verify in comments app audit). |
...
| Dirty-state guard | None (full page form) | `core:dirty-state: true` (`app.json:9-11`); Save disabled until `hasEdits` | 🟢 | Workspace improves on classic. |
| Help tab "Overview" + help sidebar | `add_help_tab` + `set_help_sidebar` w/ doc links (`options-discussion.php:21-34`) | Absent | 🔴 | No help-tab surface in the workspace generally; minor. |
...
| Plugin extension slots (`do_settings_fields('discussion','default'|'avatars')`, `do_settings_sections('discussion')`) | Renders plugin-registered fields (`options-discussion.php:211,323,326`) | Absent | 🔴 | No `core:settings.panels` slot wired for this app. Workspace-side. |
...
1. **`open`/`closed` ↔ boolean mapping.**
   ...
   - The workspace models them as `type: 'boolean'` DataForm fields and translates with `getValue: ({item}) => item.x === 'open'` / `setValue: ({value}) => ({ x: value ? 'open' : 'closed' })` (`src/apps/settings-discussion/index.js:18-34`).
...
2. **Save scope.**
   ...
   - The workspace's single Save persists only the 2 REST-exposed fields (`EntityDataForm.js:73-83` over `useEntityRecord('root','site').save`).
...
3. **Notice content differs (intentional).**
   ...
   - The workspace omits that and instead renders a REST-gap notice: "The fine-grained discussion settings … are not exposed by the WordPress REST API. Use the legacy Discussion Settings screen for those fields." (`src/apps/settings-discussion/index.js:54-59`).
   - User-visible consequence: the workspace tells the truth about the gap rather than pretending parity; arguably better UX given the limitation, but the original helper text is lost.
...
|---|---|---|---|---|
| 1 | `default_pingback_flag` | `1` | Default post settings | ⛔ [upstream] / [workspace] |
| 2 | `require_name_email` | `1` | Other comment settings | ⛔ [upstream] / [workspace] |
| 3 | `comment_registration` | `0` (`schema.php:459`) | Other comment settings | ⛔ [upstream] / [workspace] |
| 4 | `close_comments_for_old_posts` | `0` (`schema.php:500`) | Other comment settings | ⛔ [upstream] / [workspace] |
| 5 | `close_comments_days_old` | `14` (`schema.php:501`) | Other comment settings | ⛔ [upstream] / [workspace] |
| 6 | `show_comments_cookies_opt_in` | `1` (`schema.php:539`) | Other comment settings | ⛔ [upstream] / [workspace] |
| 7 | `thread_comments` | `1` (`schema.php:502`) | Other comment settings | ⛔ [upstream] / [workspace] |
| 8 | `thread_comments_depth` | `5` (`schema.php:503`) | Other comment settings | ⛔ [upstream] / [workspace] |
| 9 | `page_comments` | `0` (`schema.php:504`) | Comment Pagination | ⛔ [upstream] / [workspace] |
| 10 | `comments_per_page` | `50` (`schema.php:505`) | Comment Pagination | ⛔ [upstream] / [workspace] |
| 11 | `default_comments_page` | `newest` (`schema.php:506`) | Comment Pagination | ⛔ [upstream] / [workspace] |
| 12 | `comment_order` | `asc` (`schema.php:507`) | Comment Pagination | ⛔ [upstream] / [workspace] |
| 13 | `comments_notify` | `1` (default true) | Email me whenever | ⛔ [upstream] / [workspace] |
| 14 | `moderation_notify` | `1` (`schema.php:442`) | Email me whenever | ⛔ [upstream] / [workspace] |
| 15 | `wp_notes_notify` | `1` (`schema.php:565`, WP 6.9) | Email me whenever | ⛔ [upstream] / [workspace] |
| 16 | `comment_moderation` | `0` (`schema.php:441`) | Before a comment appears | ⛔ [upstream] / [workspace] |
| 17 | `comment_previously_approved` | `1` (`schema.php:546`) | Before a comment appears | ⛔ [upstream] / [workspace] |
| 18 | `comment_max_links` | `2` (`schema.php:451`) | Comment Moderation | ⛔ [upstream] / [workspace] |
| 19 | `moderation_keys` | `''` (`schema.php:447`) | Comment Moderation | ⛔ [upstream] / [workspace] |
| 20 | `disallowed_keys` | `''` (`schema.php:545`) | Disallowed Comment Keys | ⛔ [upstream] / [workspace] |
| 21 | `show_avatars` | `'1'` (`schema.php:482`) | Avatars | ⛔ [upstream] / [workspace] |
| 22 | `avatar_rating` | `G` (`schema.php:483`) | Avatars | ⛔ [upstream] / [workspace] |
| 23 | `avatar_default` | `mystery` (`schema.php:492`) | Avatars | ⛔ [upstream] / [workspace] |

**Closability — this blocker is both [upstream] AND [workspace]:**

- **[upstream]** — the clean fix is for WordPress core to add these to `register_initial_settings()` in `wp-includes/option.php`, exactly as the 2 existing discussion options are registered. Then every REST client (including the workspace unchanged) gains the fields. This is the "right" fix but requires a core ticket + release cycle.
- **[workspace]** — the workspace can close the entire gap itself, today, with no upstream dependency: call `register_setting( 'discussion', '<option>', [ 'type' => …, 'show_in_rest' => … ] )` for each of the 23 options on the `rest_api_init` hook (priority after `register_initial_settings`, which is 10). Because `WP_REST_Settings_Controller::get_registered_options()` reads `get_registered_settings()` live (`class-wp-rest-settings-controller.php:220`), the options then appear in the **existing** `/wp/v2/settings` endpoint — no custom endpoint, no new controller. `useEntityRecord('root','site')` would pick them up automatically. The workspace already owns a PHP layer (`includes/`) and could ship this as a small "expose discussion settings to REST" module. This is the recommended path and is strictly better than the screen-spec's suggestion of a custom `/wp-admin-workspaces/v1/options/discussion` endpoint, which would duplicate sanitization the Settings API already does.
...
- `do_settings_fields('discussion', …)` / `do_settings_sections('discussion')` plugin extensions (`options-discussion.php:211,323,326`) — arbitrary plugin-registered fields/markup; no REST representation. The workspace would surface these only via its own `core:settings.panels` slot mechanism, and only for plugins that opt in.
...
**No misuse or anti-pattern in the existing code.** The `_shared/forms/EntityDataForm.js` workspace is clean and reused correctly.
...
**P2 — Fix the screen-spec inaccuracies [workspace, docs].**
