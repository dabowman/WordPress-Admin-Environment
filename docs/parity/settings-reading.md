...
**Major gaps (blocked by API).** Of the six fields the shell renders, only **three** (`show_on_front`, `page_on_front`, `page_for_posts`, `posts_per_page` — four options, but the two page selects are one logical control) actually round-trip through `/wp/v2/settings`. The remaining options the screen owns — `posts_per_rss`, `rss_use_excerpt`, and `blog_public` — are **never registered with `show_in_rest`** in WordPress core (verified: zero `register_setting` matches across the entire `wordpress-develop/src` tree). The shell already documents the `blog_public` gap, but it still *renders* `posts_per_rss` and `rss_use_excerpt` as live form fields when in fact they are silent no-ops: the GET response omits them (so they read as `undefined` → fall back to defaults), and PATCH silently drops them (the settings controller only iterates registered options). This is worse than a missing feature — it is a control that *looks* functional and *appears* to save but discards the user's input. Combined with the omitted `blog_public` toggle and the omitted `blog_charset` field, the screen cannot reach parity through REST today; closing it requires either an upstream `show_in_rest` change or a shell-side custom REST surface.
...
| **"Syndication feeds show the most recent"** (`posts_per_rss`) | `<input type=number min=1>` + "items" unit; saved via `options.php` allowlist (`options-reading.php:183-186`, `options.php:144`) | `type:'integer'` field rendered + clamped (`index.js:113-124`) | blocked | **`posts_per_rss` is NOT in `register_initial_settings()` — never `show_in_rest`.** Field reads `undefined`→`10` default and **writes are silently discarded** by the controller. Looks functional, is a no-op. |
| **"For each post in a feed, include" radio** (`rss_use_excerpt`) | Radio Full text(0)/Excerpt(1); saved via `options.php` (`options-reading.php:188-207`, `options.php:145`) | `Edit:'radio'` 0/1 ↔ boolean (`index.js:125-141`) | blocked | **`rss_use_excerpt` NOT `show_in_rest`.** Same silent-no-op trap as `posts_per_rss`. |
...
1. **Silent-success on discarded fields (the worst divergence).**
   ...
   - Shell: `useEntityRecord('root','site').save()` PATCHes `/wp/v2/settings`. The controller's `update_item()` iterates **only** `get_registered_options()` and does `if ( ! array_key_exists( $name, $params ) ) continue;` (`class-wp-rest-settings-controller.php:145-152` + the `get_registered_options` filter at `:215-218` that skips `empty($args['show_in_rest'])`). Unregistered keys (`posts_per_rss`, `rss_use_excerpt`) are **never iterated**, so they are dropped without error. `useEntitySave` then fires the success snackbar (`useEntitySave.js:28-29`).
   - **Consequence:** a user changes "Syndication feeds show the most recent" from 10 → 25, clicks Save, sees "Settings saved." — and the value silently reverts to 10 on reload. A data-integrity / trust bug, not just a missing feature.
...
1. **`posts_per_rss` is not exposed by `/wp/v2/settings`.** `[upstream]`
   ...
   - Why it blocks: the settings controller's `get_registered_options()` skips any option with `empty($args['show_in_rest'])` (`class-wp-rest-settings-controller.php:215-218`), so the key is absent from both the GET response and the request schema (`rest_default_additional_properties_to_false` makes the schema `additionalProperties:false`). It cannot be read or written via REST.
   - Fix surface: upstream — add `show_in_rest => true, type => integer, default => 10` to a `register_setting('reading','posts_per_rss', …)` call in `register_initial_settings()`. Shell-side fallback: a custom `wp-admin-shell/v1` REST route (cap-gated `manage_options`) that wraps `get_option`/`update_option`.

2. **`rss_use_excerpt` is not exposed by `/wp/v2/settings`.** `[upstream]`
   - Evidence: zero `register_setting` match for `rss_use_excerpt`. Read by the same feed templates; default in `schema.php:425`.
   - Same mechanism / same fix surface as #1 (`type => integer` / enum `0|1`, or `boolean` with a server-side cast).
...
**P1 — Stop rendering dead fields as if they save (data-integrity bug).** `[shell]`
The `posts_per_rss` and `rss_use_excerpt` fields (`src/apps/settings-reading/index.js:113-141`) currently look functional but are silently discarded on save (see Divergence #1). Until a write path exists, either (a) remove them and extend the existing in-form notice (`index.js:157-162`) to name all three non-REST options, or (b) keep them but gate them behind a clearly-labeled "saved via legacy screen" affordance. Shipping a control that fakes success is the highest-severity finding. Also update `app.json#documentation.data.reads/writes` (`src/apps/settings-reading/app.json`) and `app.md`, which currently list `posts_per_rss`/`rss_use_excerpt` as REST-backed reads/writes — that contradicts the screen spec and the live REST schema.

**P2 — Close the REST gap for the three reading options.** `[upstream]` (preferred) or `[shell]` (stopgap)
Upstream: add `show_in_rest => true` to `register_setting` for `posts_per_rss`, `rss_use_excerpt`, and `blog_public` in `register_initial_settings()` (`src/wp-includes/option.php`). This is the clean fix and would let the existing DataForm fields work unchanged. Shell stopgap if upstream is slow: a cap-gated `wp-admin-shell/v1/reading-extras` REST route wrapping `get_option`/`update_option` for the three keys, consumed alongside `/wp/v2/settings`. `blog_public` is the most user-visible (it surfaces a dashboard-wide "Search engines discouraged" banner), so prioritize it within this item.
