...
- **REST / core-data the shell app uses:** `GET/POST/DELETE /wp-admin-workspaces/v1/user-prefs` (`includes/class-wp-admin-workspaces-prefs-rest.php:26-107`); reads `config.styles.customizable` + `config.styles` + `config['default-route']` from the resolved kernel config (`src/apps/appearance/index.js:68-108`). No `@wordpress/core-data` entities; uses `@wordpress/api-fetch` directly.
