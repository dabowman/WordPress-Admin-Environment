...
2. **Plugin `admin_notices` HTML is rendered but visually buried.** Because the workspace renders *through* `admin-header.php` (`includes/class-wp-admin-workspaces-hijack.php:358`), the `admin_notices` / `all_admin_notices` hooks **do fire** and plugins **do echo** their server-rendered notice `<div>`s into `#wpbody-content` — but those land *outside* the React `core:notices-banner` app (which only reads the JS `core/notices` store), above/behind the 100vw/100vh shell root. They are not surfaced in the shell's notice UI. **[shell]**-closeable in part; full fidelity is **[upstream]**.
...
| **`core:settings-workspace`** | (no wp-admin analog) | Yes | **Shell-native.** A workspace-on/off toggle over `/wp/v2/settings`. The parallel classic **Settings → WP Admin Shell** page (`wp-admin-workspaces.php`) is the only true wp-admin counterpart, and it writes the same option. No gap. |
...
Note the contrast with the **classic-menu bridge** (`includes/cascade/class-wp-admin-workspaces-classic-menu-bridge.php`), which *does* ingest third-party `add_menu_page()`/`add_submenu_page()` registrations into the shell menu. There is **no analogous bridge for `admin_bar_menu`**.
...
1. **[shell] Build an admin-bar bridge mirroring the classic-menu bridge.** Walk `$GLOBALS['wp_admin_bar']->get_nodes()` at render time (it's populated because the shell renders through `admin-header.php`) and serialize label/href/parent/meta into the inline config, then surface third-party nodes in `core:toolbar-actions` (or a dedicated chrome region). Accept the documented loss of `onclick`-JS nodes + live count bubbles. This closes the largest *actionability* gap without any core change. Precedent: `includes/cascade/class-wp-admin-workspaces-classic-menu-bridge.php`.
