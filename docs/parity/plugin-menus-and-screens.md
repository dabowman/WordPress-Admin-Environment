...
| **Dashboard widget** (`wp_add_dashboard_widget`) | ⚠️ Separate path | Core `wp_add_dashboard_widget` is NOT bridged; the shell has its own `wp_admin_shell_register_dashboard_widget()` (`class-wp-admin-shell-dashboard-widgets.php:83`) | A plugin's classic dashboard widget does not appear on the shell dashboard unless re-registered via the shell API. **[shell]** gap (separate audit area) |
