...
1. **Genuine third-party top-level menus** (e.g. Gutenberg, WooCommerce, Yoast): a `screens[ingested-<slug>]` pointing at `iframe:<admin-url>` (`:559-571`) **as a sibling of the workspace's own top-level screens** (NOT under the container), plus a bare `menu[ingested-<slug>]` entry (`bind_screens` fills label/icon/href later).
2. **Submenus parented to a CORE slug** the workspace doesn't mirror (e.g. a plugin adding a page under `tools.php`): grouped under a shared **`menu.ingested`** container (label "Plugins", icon `plugins`, position 200 — `ensure_container()` `:598`), with a hidden stub parent screen + children synthesized by `synthesize_child()` (`:623`).
...
`src/apps/iframe-fallback/index.js` renders `<iframe src={adminUrl + config.url}>` and on `load` calls `getChromeHideCss()` (`src/apps/_shared/iframe/chromeHide.mjs:64`) — which returns `BASE_CHROME_HIDE_CSS` (`:29-36`) hiding `#adminmenuwrap`, `#wpadminbar`, `#wpfooter`, content margins — and injects the resulting CSS into the iframe document. Same-origin (the wp-admin case) succeeds; cross-origin silently reveals chrome. The `EDITOR_CHROME_HIDE_CSS` (`:38-50`) that strips the block editor's own hub/sidebar/header is opt-in via `config.hideEditorChrome`; ingested plugin pages leave it off. The app also: keeps the iframe hidden behind a Spinner until the chrome-hide CSS is in; detects an in-iframe `wp-login.php` (session expiry) and forces a heartbeat poll so the workspace's auth modal pops; reloads on re-auth; and installs `installIframeBridge` so in-iframe `target=_parent`/`_top` clicks route up.
...
`passes_base_gates()` (`:89`) bails on `wp_admin_workspaces_is_chromeless_request()` (`:123`) — so a same-origin iframe load of a plugin page (which the browser tags `Sec-Fetch-Dest: iframe`) never re-enters the workspace → no nested-workspace recursion.
...
| wp-admin plugin-UI mechanism | Surfaced in workspace? | How | What breaks / caveat |
|---|---|---|---|
...
| **`custom_menu_order` / `menu_order` filter** | ❌ No | Bridge reads `$GLOBALS['menu']` *as populated by `admin_menu`*, but the `menu_order` sort runs later in `wp-admin/menu.php` which the bridge request never executes | A plugin's custom ordering is lost; workspace menu order = workspace.json/cascade order |
...
| **Admin-bar node** (`admin_bar_menu`) | ❌ No | The workspace renders its own toolbar/`core:site-hub`; it does **not** mirror `$wp_admin_bar`. Only the workspace's own classic-mode toggle node exists (`class-wp-admin-workspaces-classic-mode.php:124`) | A plugin's admin-bar entry (e.g. a cache-purge button, "Edit in Elementor") is invisible in the workspace. **[workspace]** gap |
| **`admin_notices` echo** | ⚠️ Iframe-local only | A plugin's `admin_notices` callback echoes into the iframe document body, so it shows *inside* the iframed page. It does NOT bridge to the workspace's `core:notices-banner` / snackbar | Notices on the dashboard / a native workspace screen (where there's no iframe) never fire — the plugin's `admin_notices` hook only runs on classic page loads, and the workspace dashboard is a React render, not a classic `index.php` load |
| **Dashboard widget** (`wp_add_dashboard_widget`) | ✅ Yes — **harvested** (#134) | `WP_Admin_Workspaces_Dashboard_Bridge` runs `wp_dashboard_setup()` (forcing the `dashboard` screen so `add_meta_box` files boxes correctly), walks `$wp_meta_boxes['dashboard']`, skips the core widgets the workspace ships native, and synthesizes a `core:dashboard-widget-classic` tile per surviving plugin widget. The tile lazily fetches the widget's `ob_start`-captured HTML from `GET /wp-admin-workspaces/v1/dashboard-widget/{id}`. The workspace `wp_admin_workspaces_register_dashboard_widget()` API still exists for first-class native tiles. | A plugin's classic dashboard widget now surfaces on the workspace dashboard automatically (captured HTML, admin trust). Limitation: JS-driven widgets degrade to static HTML; per-tile "Open classic dashboard" iframe is the fidelity fallback. |
...
- **Separators: dropped (correct).** Bridge skips separator rows (`:224-227`); the workspace's own menu IA supplies structure.
- **`custom_menu_order`: lost (real).** See matrix. The reorder filter fires in `wp-admin/menu.php`, never executed in the bridge's data-gathering request. **[workspace]**

### 3.4 Settings-API specifically

A third-party `add_options_page()` + Settings API page:
- **Appears** under the workspace's "Plugins" container (`menu.ingested`), NOT under the workspace's Settings host — because the bridge groups all core-parented submenus under one container (`classic-menu-bridge.php:516`), and `options-general.php` is a core parent. So a plugin settings page is reachable but mis-placed (user looks under Settings, finds it under Plugins). **[workspace]** placement gap.
...
1. **[upstream] A plugin's Settings-API options are not in REST.** `register_setting`'s `show_in_rest` defaults to `false` (`option.php:2994`). The workspace's native settings apps (`core:settings-*`) work only because *core* options opt into REST (`option.php:2818,2828,2838,…`). A third-party plugin's options are generally invisible to `wp/v2/settings`, so a **native React rebuild of a plugin settings page is impossible** without the plugin author adding `show_in_rest`. The iframe is the only faithful path. This is fundamental, not a workspace shortcoming.
...
3. **[upstream] admin-ajax-only actions.** Many plugins ship `wp_ajax_<action>` handlers with no REST equivalent (`admin-ajax.php`, nonce-gated). `@wordpress/api-fetch` can technically hit admin-ajax, but there's no schema, no discovery, and the nonces are page-localized. Inside the iframe these work natively because the plugin's own JS carries the nonce; outside, the workspace can't enumerate or call them. The workspace does not (and cannot generically) bridge these.
4. **[upstream] `$GLOBALS['menu']` / `$submenu` are not exposed via REST.** The workspace reads them server-side (PHP bridge) because there is no `wp/v2/admin-menu` endpoint. This is fine for the PHP-side bridge, but means **a headless/remote consumer of the workspace config cannot reconstruct the plugin menu** — only an in-`wp-admin` request populates the globals (they're built by `admin_menu`, which only fires on admin page loads). A REST endpoint returning the assembled menu would let the workspace (or any client) get plugin menus without a live admin request.
5. **[workspace] Admin-bar nodes (`admin_bar_menu` → `$wp_admin_bar`) are not bridged.** `WP_Admin_Bar` *is* introspectable server-side (`$wp_admin_bar->get_nodes()`), so this is closable workspace-side without upstream help — the workspace just doesn't do it today.
6. **[workspace] `admin_notices` are not bridged.** The `admin_notices` action fires during classic page render and echoes HTML; on a native workspace screen there's no such render. Capturing them would require running the action in a buffer server-side and shipping the output — doable workspace-side (with sanitization concerns) but not done.
7. **[workspace] Menu icon `data:`/image-URL harvesting.** `map_icon` punts on non-Dashicon icons (`classic-menu-bridge.php:476`). The SVG/URL is right there in `$menu[6]`; the workspace could register it into the icon system. Explicitly deferred (`:46`).
8. **[workspace] Position / `menu_order` fidelity.** The numeric `$menu` key (and the `menu_order` filter result) is available server-side; the bridge just doesn't carry it. Closable workspace-side.

---

## Prioritized recommendations

1. **[workspace, high] Carry `$position` from the `$menu` numeric key into the ingested item's `position`.** `scan()` already iterates `$menu`; capture `$id` (the array key) and emit `position => (int) round($id)` so ingested items land near where the plugin asked. Closes the most visible drift. (`classic-menu-bridge.php:213`)
2. **[workspace, high] Place core-parented plugin submenus under the matching workspace menu, not a generic "Plugins" bucket.** A plugin `add_options_page()` should nest under the workspace's Settings menu; `add_management_page()` under Tools. Map the core parent slug → the workspace's menu id instead of always `menu.ingested`. (`classic-menu-bridge.php:516`)
3. **[workspace, high] Bridge `admin_bar_menu` nodes into the workspace toolbar.** Read `$wp_admin_bar->get_nodes()` server-side after `admin_bar_menu` fires and contribute them as toolbar items. Plugin admin-bar actions (cache purge, "Edit with…") are currently invisible. Fully closable without upstream.
4. **[workspace, medium] Harvest `data:`/image-URL menu icons.** Register the embedded SVG / image URL into the icon registry so WooCommerce/Yoast/Elementor render with their real glyph instead of the generic `menu` icon. (`classic-menu-bridge.php:476`)
5. **[workspace, medium] Surface Screen Options / Help affordance under `core:default`, not just `core:desktop`.** Sub-system 12 already detects + posts the panels (`chromeless-bridge.php:624`); wire the default-engine iframe-fallback's parent-side listener (or a region titlebar control) to consume `wp-admin-workspaces-screen-meta` the way `core:desktop-iframe` does. The data is already on the wire.
6. **[workspace, medium] Bridge `admin_notices` for native workspace screens.** Buffer the `admin_notices`/`all_admin_notices` output server-side, sanitize, and feed `core:notices-banner`. Especially the dashboard, where there's no iframe to carry them.
7. **[workspace, low] Reproduce the first-submenu-equals-parent landing semantics** only if a concrete plugin breaks — current behavior is cleaner and the parent screen URL already resolves correctly.
8. **[upstream, strategic] Propose a `wp/v2/admin-menu` REST endpoint** returning the assembled `$menu`/`$submenu` (post-cap-prune, post-`menu_order`). This is the single upstream change that would let the workspace — and any headless client — get plugin menus *with correct ordering and icons* without scraping PHP globals during a live admin request, and would also fix the cross-request / headless reconstruction gap (#4 above).
9. **[upstream, evangelism] Push plugin authors toward `show_in_rest` on `register_setting` and toward REST endpoints over admin-ajax.** This is the only path from "iframe parity" to "native React parity" for plugin settings/data. Document it in the plugin-author guide as the prerequisite for first-class (non-iframe) workspace UI.
