...
1. **Plugin UPDATE (version bump / "update now" / bulk update) — no REST endpoint. [upstream]**
   `WP_REST_Plugins_Controller` exposes list / create(install-from-slug) / update(status only) / delete (controller:36-102). `update_item` changes *activation status* only (controller:461-485) — it never runs the upgrader. The actual version bump runs through admin-ajax `wp_ajax_update_plugin` (`wp-admin/includes/ajax-actions.php:4618`), which is `check_ajax_referer('updates')` nonce-gated and delegates to `Plugin_Upgrader::bulk_upgrade()`. There is no `register_rest_route` for plugin upgrades anywhere in `src/`. **Missing surface:** a REST route that runs `Plugin_Upgrader` for an installed plugin. Closing it shell-side ([shell]) would require either a custom `/wp-admin-shell/v1/plugin-update` proxy endpoint wrapping `Plugin_Upgrader`, or iframing `update.php?action=update-selected`.
...
- **[upstream] Auto-update toggle via REST.** Register `auto_update_plugins` with `show_in_rest` (or a dedicated route) so the toggle column + bulk enable/disable are reachable. Verified not registered today. Interim [shell]: custom `/wp-admin-shell/v1/plugin-auto-updates` endpoint.
- **[upstream] Plugin-directory proxy.** The Add-New flow is the single biggest parity gap and is entirely blocked on `plugins_api` not being REST. Either expose a core REST wrapper for `plugins_api`, or [shell] ship a `/wp-admin-shell/v1/plugins-directory` proxy. This unblocks browse tabs, search, cards, ratings, install counts, compatibility, and the More-Details modal. Without it the shell cannot install anything except by typing a known slug.
