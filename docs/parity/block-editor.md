...
| **Dirty-state → NavigationGuard** | core's `beforeunload` (inside iframe) | ✅ wired via bridge — chromeless bridge sub-system 15 relays `core/editor` `isEditedPostDirty()` as `wp-admin-shell-dirty-state`; `index.js` feeds it into `useDirtyState(regionId, isDirty, {blocksNavigation:true})` | ✅ wired (`useDirtyState(regionId, hasEdits, {blocksNavigation:true})`) | editor ✅ / simple ✅ | A sidebar click while dirty now hits NavigationGuard's confirm instead of silently discarding (fixed issue #102) |
