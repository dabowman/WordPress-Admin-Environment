...
1. **Autosave semantics — simple-editor mutates the live record where core would write a safe autosave.** ✅ **RESOLVED (issue #101).**
   ...
   - Remaining gap (not data-loss): the autosave is write-only — there is still no `modified_gmt` "newer autosave" recovery banner on load (see blocker #3), and the interval is the shell's 2s debounce rather than core's 60s.
...
1. **Post-lock acquire + 10-second heartbeat refresh — no REST surface.** `[upstream]`
   ...
   - Shell impact: `simple-editor` ships zero lock handling → concurrent editors silently clobber. `core:editor` inherits core's lock because it iframes the real `post.php`. A native rebuild **cannot** acquire/refresh a lock through REST/core-data; it would need a custom `wp-admin-workspaces/v1` endpoint wrapping `wp_set_post_lock`/`wp_check_post_lock`, or to poll heartbeat directly. Tag upstream because the real fix (a REST lock surface) belongs in core.
...
4. ✅ **DONE (issue #102).** Added session-expiry recovery to `core:editor`, ported from `iframe-fallback`: `onIframeLoad` detects the login form + masks it + forces a heartbeat poll, and a `heartbeat-tick` listener reloads the frame on re-auth. A mid-edit timeout now surfaces the shell wp-auth-check modal instead of a stripped login form. (`src/apps/editor/index.js`.)
