...
| **Autosave (every 60s → /autosaves)** | `AUTOSAVE_INTERVAL`; published→`POST .../autosaves` (autosaves-controller), draft→PUT parent | core's (iframed) | **2s debounce, status-gated** (`index.js`, `autosave.mjs`): draft/pending→`save()` PUT parent; published/private/scheduled→`POST .../autosaves` | editor ✅ / simple 🟡 | **Data-integrity fixed (issue #101)** — live published record no longer clobbered; still no 60s interval and no recovery banner |
...
1. **Autosave semantics — simple-editor mutates the live record where core would write a safe autosave.** ✅ **RESOLVED (issue #101).**
   ...
   - **Fix:** the debounced autosave is now status-gated (`autosaveTarget()` in `src/apps/simple-editor/autosave.mjs`): draft / auto-draft / pending posts still flush via `save()` (PUT parent, matching core's controller); published / private / scheduled posts route to `POST /wp/v2/{type}/{id}/autosaves` and leave the live record untouched, only flushing live on an explicit Update. Unknown statuses fail closed to the autosaves path.
