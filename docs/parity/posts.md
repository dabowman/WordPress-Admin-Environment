...
> Audited 2026-05-29 against WordPress 7.0 core. Shell app: `src/apps/posts/`. Classic counterpart: `src/wp-admin/edit.php`, `src/wp-admin/includes/class-wp-posts-list-table.php`, `src/js/_enqueues/admin/inline-edit-post.js`.
...
| **Status column** | Not a standalone column in classic — status shown via badges + date label | `Text` mapping status→label; `index.js:93` | ➕ divergence | Shell adds a dedicated Status column classic doesn't have (reasonable). |
...
| **Comments column (bubble)** | `column_comments()` + `comments_bubble()` — pending/approved count linking to comments screen; `:1261` | absent | ⛔ blocked (count) | `comment_count` is **not** a REST field on `/wp/v2/posts`; only an embeddable `replies` link (`wp/v2/comments?post=ID`). [upstream] for a scalar field; [shell] possible via per-row count. |
| **Custom plugin columns** | `manage_{post_type}_posts_columns` + `manage_…_custom_column` hooks; `:738` | via dataView field cascade + `register_rest_field` | 🟡 partial | Shell has a column-extension path (dataView fields), but plugin columns that compute server-side HTML have no bridge unless exposed via REST. |
...
| **Row action: View** | published posts → permalink; `:1555` | `view` callback → `window.open(item.link)`; gated `status==='publish'`; `index.js:253` | 🟡 partial | Matches for published. Classic also offers **Preview** for draft/pending/future via `get_preview_post_link()` — shell has no Preview. |
...
| **Bulk action: Bulk Edit** | inline multi-row field editor; `bulk_edit_posts()`; `:440`, `inline_edit()` | absent | ⛔ blocked (component) | No inline bulk-edit UI. Underlying field writes (status/author/sticky/parent/format/comment_status/categories/tags) are **all REST-doable** via batched `updateEntityRecord`. [shell] (build a DataForm-driven bulk panel). |
...
| **Help tabs** | Overview / Screen Content / Available Actions / Bulk actions + sidebar; `edit.php:249` | absent | ❌ missing | No help-tab equivalent in the shell. |
| **Capability gating (screen)** | `ajax_user_can()` = `edit_posts`; `:144` | screen `permissions` + 4-layer kernel gating | ✅ full | Gated at screen level via admin.json. |
| **Capability gating (per-row action)** | per-post `current_user_can('edit_post'/'delete_post', id)`; `:1479`, `:1512` | static `eligibleWhen` by status only; `app.json` actions | 🟡 partial | Shell gates by *status*, not per-row capability. REST exposes `wp:action-*` link rels (`action-sticky`, `action-assign-author`, `action-publish`) for per-row caps, unused. |
...
| **Extensibility: row/bulk action hooks** | `post_row_actions` / `page_row_actions` / `bulk_actions-*` | dataView `actions` cascade + filters | 🟡 partial | Reachable via admin.json/filters, but PHP action-link hooks aren't ingested. |
...
1. **Status counts: filter-dropdown labels vs. subsubsub tab strip.**
   ...
   - Shell folds counts into the **status filter dropdown** option labels via `withElementCounts()` (`src/apps/_shared/dataviews/buildFields.mjs:105`) seeded by `useEntityElementCounts` (`src/apps/posts/index.js:153`).
...
2. **Counts cost: one `wp_count_posts` query vs. N REST requests.**
   ...
   - Shell fires one `GET ?status=X&per_page=1&_fields=id` per status value (6 values in `STATUS_VALUES`) and reads `X-WP-Total` (`useEntityElementCounts.js:44-58`).
...
3. **"Mine" auto-scope for low-privilege users is not replicated.**
   ...
   - Shell always queries `status: config.status || 'any'` with no author scoping (`index.js:119`).
   - **Consequence:** a Contributor in the shell sees the full readable list by default rather than just their own, a subtle authorization-UX divergence (the REST layer still filters by *read* capability, so they won't see private others'-posts content, but the default framing differs).

4. **Restore semantics: shell would restore to draft, classic restores to previous status.**
   - Classic `wp_untrash_post()` restores to the pre-trash status stored in `_wp_trash_meta_status` (`edit.php:147-167`).
   - Shell's `restore` action is currently inert (no callback in `index.js`), but the documented intent (`app.md:91`, `docs/screens/posts.md:105`) is `status: 'draft'`.
...
5. **Trash UX: confirm modal vs. immediate nonced link.**
   ...
   - Shell opens a confirm modal first (`createBulkConfirmModal`, `index.js:199`) and shows no Undo.
   - **Consequence:** shell trades wp-admin's "act-then-undo" for "confirm-then-act"; arguably better, but no Undo path exists (spec `docs/screens/posts.md:308` wants Undo).
...
6. **Editor navigation only handles `post`/`page`.**
   ...
   - **Consequence:** binding `core:posts` to `wp_block` (as `app.md:7` says some shells do) produces broken edit links. Documented limitation (`app.md:90`).
...
1. **Post locking — "{user} is currently editing".** `[upstream]`
   ...
   - **To close:** upstream a `wp:lock`/`_edit_lock` read field or a `/wp/v2/posts?_fields=lock` computed field (+ a way to refresh). Until then the shell cannot warn that a post is being edited.

2. **Per-row comment count (the comment bubble).** `[upstream]` (preferred) / `[shell]` (workaround)
   ...
   - **To close:** `[upstream]` register a `comment_count` (and ideally a pending count) REST field on posts; or `[shell]` fire a `GET /wp/v2/comments?post=ID&status=approved&per_page=1` per row and read `X-WP-Total` (expensive at list scale — one request per visible row).

3. **First-class status counts.** `[shell]` (worked around) / `[upstream]` (ideal)
   - Classic gets all status counts in one `wp_count_posts()` call (`:299`).
   - **Missing surface:** no REST endpoint returns post counts grouped by status. The shell's `useEntityElementCounts` already works around this with N `per_page=1` + `X-WP-Total` requests.
   - **To close:** `[shell]` accept the N-request cost (current), or `[upstream]` add a counts endpoint (e.g. `GET /wp/v2/posts/counts`).

4. **Quick Edit / Bulk Edit inline UI.** `[shell]` (the UI), not API-blocked for data
   ...
   - **What's missing is the *UI primitive*:** the shell does not surface DataViews' inline edit, and there is no inline-row or bulk-panel editor. This is a `[shell]` build (a DataForm-driven modal / panel), not an API gap. Tagging it "blocked (component)" in the matrix means *the affordance is absent*, not that the API can't do it.

5. **Preview link for unpublished posts.** `[shell]` mostly
   - Classic builds a nonced preview URL via `get_preview_post_link()` for draft/pending/future (`:1545`).
   - REST does not hand you a ready-to-use preview URL; the shell would need to construct `?preview=true&preview_id=ID&preview_nonce=…` (the preview nonce is generable client-side via the editor's existing flow) or route into the editor's preview. Minor and `[shell]`-closeable, with a small nonce wrinkle.
...
Prioritized. Each: what / why / where / shell-vs-upstream.

**P1 — highest parity value, mostly shell-side**

1. **Wire the already-declared Trash-variant actions (Restore, Delete Permanently) + a Trash view.** They are declared in `app.json` (`trash` variant) but have **no callbacks** in `index.js`, so they're inert. Add `restore` (`updateEntityRecord(status:'draft')`) and `delete-permanent` (`deleteEntityRecord(..., { force:true })`) callbacks, plus a way to reach the Trash filter. *Where:* `src/apps/posts/index.js` (callbacks), shells' dataView. *Shell-side.* Note restore-to-draft caveat (blocker #4).
2. **Build Bulk Edit.** The single biggest functional gap. A DataForm-driven panel/modal that, on confirm, batches `updateEntityRecord` per selected row over the changed fields only (status / author / sticky / parent / format / comment_status / categories / tags — all REST-writable). *Where:* new `src/apps/posts/` component + `_shared/forms`. *Shell-side* (data fully supported).
3. **Add the status/Mine view-tab strip (or restyle counts as tabs).** Replace/augment the filter-dropdown counts with a one-click tab strip incl. **Mine** (`author=current_user`) and **Sticky** (`sticky=true`). *Where:* shell config + possibly an engine-side tab affordance or DataViews view list. *Shell-side*; the count engine already exists.
4. **Sticky support (filter + badge + Quick/Bulk toggle).** `sticky` is fully REST-exposed. Add a sticky filter, a Sticky state badge in the title cell, and sticky in Bulk/Quick Edit. *Where:* `index.js` field renderers + bulk panel. *Shell-side.*

**P2 — meaningful gaps, shell-side with some nuance**

5. **Quick Edit (inline single-row edit).** Adopt DataViews editable fields or a per-row DataForm modal saving via `PUT /wp/v2/posts/{id}`. *Where:* `src/apps/posts/`. *Shell-side* for data; DataViews-feature-adoption for UI.
6. **Column filters: date (month), category, format, author.** All REST-supported (`after`/`before`, `categories`, `format`, `author`). Map a month picker to an ISO range. *Where:* `index.js` `queryArgs` + DataViews filters config. *Shell-side.*
7. **Comment-count bubble.** *Shell-side* interim: per-row `GET /wp/v2/comments?post=ID&per_page=1` + `X-WP-Total` (costly). *Upstream-preferred:* register a `comment_count` REST field. *Where:* `index.js` + `[upstream]` `class-wp-rest-posts-controller.php`.
8. **Per-row capability gating.** Consult `wp:action-*` link rels (`action-publish`, `action-sticky`, `action-assign-author`) to enable/disable per-row actions and suppress checkboxes on non-editable rows, instead of status-only `eligibleWhen`. *Where:* `buildActions` eligibility overrides reading `_links`. *Shell-side.*
9. **State badges + status-aware date label + Missed-schedule warning.** Reproduce `_post_states` (Sticky/Draft/Pending/Private/Front Page/Posts Page) in the title cell (front-page/posts-page need a `/wp/v2/settings` cross-read) and the date column's status-aware label. *Where:* `index.js` renderers. *Shell-side.*
10. **Preview link for draft/pending/future.** Construct a preview URL or route into the editor preview. *Where:* `index.js` `view`/new `preview` action. *Shell-side* (minor nonce wrinkle).
11. **Hierarchical Pages tree.** Pages currently render flat. DataViews has no native tree; either build an indented renderer keyed on `parent`/`menu_order` or accept the flat divergence. *Where:* `index.js` when `postType==='page'`. *Shell-side*, but needs a tree affordance (possible `[upstream DataViews]` ask for hierarchy support).

**P3 — polish / lower value**

12. **Persisted Screen Options (per-page + column visibility).** Persist DataViews page size + visible fields as a per-user preference (e.g. via `@wordpress/preferences` or user meta) to mirror `edit_{post_type}_per_page`. *Where:* `useEntityDataView` + a prefs store. *Shell-side.*
13. **Empty Trash bulk button.** Query trashed ids, batch `force` delete. *Where:* `index.js`. *Shell-side.*
14. **Undo snackbar after trash.** Restore the trashed ids on click; mirrors wp-admin's "Move to trash · Undo". *Where:* `createBulkConfirmModal`/`index.js`. *Shell-side.*
15. **Post-type-specific empty + error states, help content.** Use the post type's `not_found`/`not_found_in_trash` labels; add a load-error banner; consider a help affordance. *Where:* `index.js`. *Shell-side.*

**Upstream (WordPress / REST) asks**

16. **`[upstream]` Expose post-lock status via REST** (blocker #1) — a read field or computed `_fields=lock` returning holder + freshness, ideally with a refresh path. Without it, "X is currently editing" is impossible in the shell.
