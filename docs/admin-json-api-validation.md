# `workspace.json` Schema — REST API Validation
...
**Schema adjustment needed: None.** The `config.postType` field is the right abstraction. One consideration: the schema should probably also support `config.status` (default filter) and `config.orderby` (default sort) for cases where a workspace config wants to show "Draft Posts" as a distinct nav item.
...
The workspace doesn't need to implement any REST API calls for the editor — it just mounts the `@wordpress/edit-post` React application and passes it a post ID. The editor handles its own data layer.

**Schema adjustment needed: Minor.** The editor application needs to receive a post ID from the workspace's routing system. When a user clicks "Edit" on a post in the DataViews list, the workspace navigates to `editor` with a route parameter like `/editor/post/42`. The `config` for the editor application should probably declare which post types it handles:
...
Like the block editor, the site editor manages its own data layer. The workspace just mounts it.

**Schema adjustment needed: None for the POC.** However, embedding the full site editor inside a workspace is the most complex mounting challenge because `@wordpress/edit-site` currently assumes it owns the entire viewport and manages its own navigation (the sidebar browser, template list, etc.). The POC might need to mount it in a near-full-screen mode within the workspace's content region, or it might need a wrapper that constrains it. This is an implementation challenge, not a schema issue.
...
This doesn't use the REST API at all — it renders an existing wp-admin page inside an iframe in the workspace's content region. The iframe source URL resolves relative to `wp-admin/`.

**Considerations:**
- **Authentication** — since the workspace page is served from the same WordPress install, the user's cookie auth carries into the iframe. No additional auth needed.
- **Navigation conflicts** — the iframed wp-admin page has its own admin menu and admin bar. The workspace should inject CSS into the iframe to hide `#adminmenuwrap`, `#wpadminbar`, and `#wpfooter`, leaving just the content area. This is a common pattern used by plugins that embed wp-admin pages.
- **Link interception** — clicks on links inside the iframe that navigate to other wp-admin pages should ideally be intercepted and handled by the workspace's routing, not by the iframe navigating internally. For the POC, letting the iframe handle its own navigation is acceptable; link interception is a refinement.
...
**Schema adjustment needed: None.** The `iframe:{url}` pattern is correct as designed. It's the bridge that makes the POC immediately useful for screens that don't have native workspace applications yet.
...
## Authentication in the workspace context

The workspace renders as a WordPress admin page (registered via `add_menu_page` or similar, served at `wp-admin/admin.php?page=workspace`). This means:

- The user is already authenticated with a WordPress cookie session.
- The nonce for REST API calls is available via `wp_create_nonce('wp_rest')`, passed to the workspace's JavaScript via `wp_localize_script` or the `wpApiSettings` global.
- All REST API calls from the workspace use **Cookie + Nonce authentication** — the standard pattern for same-origin admin JavaScript.
...
The workspace should use `@wordpress/api-fetch` (the WordPress fetch wrapper that automatically handles nonce inclusion, path resolution, and middleware). This is what the block editor uses and it handles all the auth plumbing.

---

## Data fetching strategy

The workspace runtime should use `@wordpress/data` with the `core` store (`@wordpress/core-data`) rather than raw `fetch` calls. This gives us:
...
```
workspace.json config           →  Workspace reads postType from config
  ↓
Workspace passes postType       →  DataViews component
  ↓
...
The schema doesn't currently declare command palette configuration, but it should interact with commands implicitly. When the workspace loads an `workspace.json` that only exposes Posts, Pages, and Media, the command palette should:
...
- **Not** show commands for templates, navigation, plugins, or other screens not in the current workspace's application set
- Still show navigation commands for all registered applications ("Go to Posts", "Go to Media")

Implementation approach: the workspace runtime registers commands for each application in the `workspace.json`. Applications that aren't registered don't get commands. This uses the existing `@wordpress/commands` API — the workspace just controls which commands are registered based on the config.
