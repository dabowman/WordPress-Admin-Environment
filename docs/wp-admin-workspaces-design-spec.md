...
2. **Programmatic path.** Plugin calls `wp_admin_workspaces_register_app( $manifest_array_or_path )` from PHP, or the JS equivalent.
...
```jsonc
{
  ...
  "styles": [
    ...
    { "handle": "wp-admin-workspaces-dataviews",   "src": "build/dataviews.css",
      "deps":   [ "wp-components" ] }
  ]
}
```
...
2. **Chrome extensions** — shell-only slots as `--wp-admin-workspaces--chrome--*`.
...
Resolver caching (§4.4.3 of prior draft) carries forward unchanged: per-`(active_workspace, current_user_id, role)` cache key, hash-of-origins invalidation, `WP_Object_Cache` + transient layers.
...
Capability resolution uses `core-data`'s `canUser()` for entity-level checks; custom caps resolve via `/wp-admin-workspaces/v1/can/{capability}` REST endpoint. Per-request cached.
...
6. **Register a complete workspace.** A plugin programmatically registers an entire `workspace.json` (e.g., based on user role at runtime). `wp_admin_workspaces_register_workspace( $slug, $admin_json )`.
...
14. **Register a dashboard widget.** `wp_admin_workspaces_register_dashboard_widget( $id, $args )`. The widget primitive is "an app placed in a grid slot": the registry contributes a `screens[<target>].apps[]` entry with `slot: "grid"` (target screen defaults to `dashboard-widgets`, overridable via `$args['screen']`). **Override flavor.** `$args` carries placement/size only (`position`, `defaultSize`, `minSize`, `title`, `hidden`) for an app whose manifest is already registered; `hidden: true` contributes an array tombstone so the entry is removed from the resolved screen. **Standalone flavor.** When `$args` includes a `script` handle (plus optional `role`, `title`, `capabilities`, `slotHints`), the function additionally synthesizes a minimal app manifest so a single PHP call covers both the manifest entry and the placement — useful when a mu-plugin ships a widget without a full `apps/{name}/app.json` on disk. **Cascade resolution.** Contributions enter through the `plugin` origin at priority 5 and merge with any workspace.json `screens[id].apps[]` declarations via the normal id-keyed array merge — a workspace.json entry with the same id wins. At render time a grid-host app composes each surviving entry per-property over the referenced manifest's `slotHints` block; sizes translate to `grid-column: span N` / `grid-row: span M`, explicit positions to `grid-row/column-start`. **No grid host ships in `main`** — the reference host (`core:dashboard-host` + its `composeScreenWidgets({ screen, manifests })` compiler) is parked on `archive/native-apps`, and the bundled workspaces serve the dashboard through the iframe escape hatch — so registrations render only when the active workspace mounts a grid-host app (custom or un-parked) for the target screen. Cap gating, theming, and DS-mismatch warnings flow through the existing 4-layer mount path because widgets are apps — there is no separate widget gating layer. The `core:dashboard-grid` region template ships with the `core:dynamic-children` platform service so an engine compositor can drive widget mounts as runtime-mutable child regions. Drag-to-reorder and per-user widget ordering are out of scope for this primitive.
...
add_action( 'plugins_loaded', function () {
  wp_admin_workspaces_register_engine( __DIR__ . '/engines/material/engine.json' );
  wp_admin_workspaces_register_app(    __DIR__ . '/apps/material-docs-list/app.json' );
  wp_admin_workspaces_register_app(    __DIR__ . '/apps/material-doc-editor/app.json' );
  wp_admin_workspaces_register_workspace( 'material-docs',
...
import { registerIcons } from 'wp-admin-workspaces/runtime/config/iconMap';
...
3. **Region template registration from plugins.** v1 ships engines with their templates baked in. Plugins extending an engine with new templates use `wp_admin_workspaces_register_template( $engine_id, $template )` — but the engine has to know how to render the template's layout. How does an engine discover and render plugin-contributed templates? Open. Needs design before v2.
...
**Hijack.** When active, `WP_Admin_Workspaces_Hijack` (admin_init priority 0)
...
reaches its own handler. The legacy `admin.php?page=wp-admin-workspaces` entry
is removed.
...
`wp_admin_workspaces_hijack_allowlist` filter. Plugin `admin.php?page=*` pages are
...
`wp_admin_workspaces_is_chromeless_request()` (browser `Sec-Fetch-Dest: iframe`
OR the explicit `?wp_admin_workspaces_chromeless=1` flag the bridge sets). Both
...
- **`wp_admin_workspaces_enabled`** option (default true,
...
(`WP_Admin_Workspaces_Admin_Routes::legacy_map()`).
