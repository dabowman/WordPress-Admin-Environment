...
WP Admin Shell is a configurable admin environment for WordPress. The traditional `wp-admin` is a single, fixed UI shaped over twenty years to serve all audiences. As a result, it now only serves one main user well: the technical site administrator. WP Admin Shell separates the **admin interface** from the **WordPress system**, then lets a single declarative document — `admin.json` — describe an admin experience tailored to a specific role, brand, or workflow. Admin.json and the shell environment is to the back-end what theme.json and blocks is to the front-end. It's an easy config layer and system of interaction that provides powerful control and customizability within what core offers as well as a platform to use WordPress in new ways and customize beyond what core offers. 

A site can ship multiple `admin.json` files. The same WordPress install can present a focused, customizable user experience based on user roll, or user preference — without forking core, without custom plugins per surface, and without sacrificing the REST API contract that powers everything. It also provides a stable framework for further cusomization. If someone wanted to write a custom shell to provide a very different or specific admin experience the shell framework would make that possible in a more straightforward way. It could also make react-based admin screens and experiences much more portable and configurable by providing a minimal, configurable shared framework. 
